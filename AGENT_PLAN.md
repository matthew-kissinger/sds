...
> **Superseded by `NEXT_SESSION.md` at repo root.** That file is the authoritative brief for the overnight agent executing the Cloudflare migration. What follows here is the short summary; do not execute from this section alone.

### 10.0 · Policy calibration (updated 2026-04-24)

The original roadmap wrote in Cycle 1 rollback mode specified 7-day soak, dedicated staging subdomain, elaborate gate ceremony, and a runtime-switchable flag. That has been retired. SDS is a solo side project with no active user base to protect; the cost of a small prod regression is five minutes of fix-up, not a page-out. Pragmatic policy:

- Ship direct to prod. No staging subdomain.
- Use the Geckos droplet as a parallel fallback for 3-7 days, then destroy it.
- Keep the POSTMORTEM's list of the 7 Cycle-1 bugs as a "don't repeat" checklist, not as gates for a soak window.
- Rollback = `git revert <commit> && git push` or a DNS record swap. No runbook ceremony.

### 10.1 · Three-phase execution plan

**Phase A - Verify / recreate CF resources.** Source `~/.config/mk-agent/env`, run `npx wrangler whoami`, check for `sds-worker`, `sds-db`, `sds-frontend`. Recreate anything missing per `docs/c-retry/cf-recreate.md`. Set `CF_API_TOKEN`/`CF_ACCOUNT_ID` repo secrets.

**Phase B - Build the worker.** New `worker/` dir with TypeScript Workers entry, `RoomDO.ts` (port of `server/GameSimulation.js` at 60 Hz), `LobbyDO.ts`, D1 schema, REST + WS endpoints matching `docs/c-retry/contract.md` + `docs/c-retry/protocol-v2.md` exactly. Must include (these were missing in Cycle 1): `POST /api/rooms`, `/api/score` writing `players.best_*`, `/api/register` inserting rows, identity in WS hello handshake per `authority.md`, coop routing not collapsed to competitive, `competitiveGates` on every state frame, `interpolatingToClient` on dog state, `vx/vz` on sheep state, `sheepRetired` in coop state. Unskip `tests/integration/flow.spec.ts` steps as each endpoint lands.

**Phase C - Client swap + deploy + iterate.** Swap `js/NetworkManager.js` from Geckos to native WS + MessagePack pointing at the worker. Keep all client-side interpolation improvements already on main (velocity extrapolation, adaptive jitter buffer, dog blend, frame-rate-independent camera). `wrangler deploy`, `wrangler pages deploy`, bind domain. Play the live site. Fix what breaks. Commit each fix. Restore `.github/workflows/deploy.yml` pointing at CF Pages. Leave droplet running for a few days.

### 10.2 · What's already on main (as of 2026-04-24)

- `docs/c-retry/contract.md`, `protocol-v2.md`, `authority.md`, `b2-audit.md`, `cf-recreate.md`, `cycle-1-audit.md`
- `tests/integration/` (vitest + msgpack + ws two-client harness, 26 self-tests green, 7 flow tests skip-gated)
- `tests/sim-baseline/` (deterministic 60 Hz reference traces)
- `tests/e2e/smoke.spec.ts` (Playwright)
- Client-side MP polish (velocity extrapolation, adaptive jitter, dog blend) landed in PR #27
- Player-facing fixes (camera, sprint animation, grass density, mountain LOD, per-system triangle panel) landed in PRs #22-#26

### 10.3 · What's explicitly out of scope for the overnight session

- Separate staging environment
- 7-day soak window
- Feature flag plumbing for the backend (plain deploy-or-revert is simpler)
- Rollback runbook beyond `git revert` / DNS swap
- Destroying the droplet (do it after 3-7 days of stable CF prod, not during cutover)

### 10.4 · Historical roadmap (superseded, retained for context)

The pre-2026-04-24 revision of this section enumerated Tracks C0, C1'-C5', F', G with mandatory gates and a soak window. That text has been removed to avoid confusion; the history is in git. If anyone needs the old version, `git show 62430fa~1:AGENT_PLAN.md` has it.

---

*End of cycle plan. Overnight agent: see `NEXT_SESSION.md`.*
