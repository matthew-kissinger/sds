...
## Cycle 97 - golden baselines re-pinned to the shipped look; KTX2 is GO on merits, integration teed up (2026-06-14)

Matt ran Cycle 97 with "complete autonomously and ship - no need for human visual check - I will test in prod." That delegates the paired Phase 1 visual sign-off to his live validation and unblocks the queue items whose only gate was the look approval. It does not delegate the product calls, the version/launch act, or the upstream block.

- **Look-approval gate delegated to prod validation.** The shipped look on sheepdogsim.com (Cycle 92 frame-floor + trunk-split + pill removal, Cycle 95 NSL fixes) is the approved baseline; Matt's prod validation of the Cycle 95 A/B/C/E/D/F items is the backstop. If prod validation rejects an element, it becomes a fast-follow and the affected goldens re-capture against the corrected look.
- **Golden suite re-baselined** (`tools/validation/golden/`, 12 cells: field/rolling-hills/open-country × 2 ToD × 2 camera). The 2026-05-16 baselines were obsolete (Cycle 91 reframed the follow camera, Cycle 92 changed impostors; every cell diffed near-zero SSIM). A `--diff` smoke proved headless WebGPU renders healthy frames; the captures were visually inspected before re-pinning (stale beats wrong). Two fixes landed with the re-pin: (1) the dead `konveyorRocks` codename param was removed from the harness URL (no `js/`/`shared/` consumer since the Cycle 87 konveyor retirement), so the goldens reflect the default production rock path; (2) a capture-determinism fix in `GrassSystem.js` freezes the grass wind/LOD clock under `visualGolden` (production byte-identical). The suite was intermittently flaking one all-grass aerial cell to ~0.67 SSIM because grass wind is wall-clock-driven; with the clock frozen, two consecutive `--diff` runs pass clean. NSL is not in the matrix yet (streamed foliage + 14s cold load make a single-frame headless capture non-deterministic); a streaming-aware NSL capture is the follow-up.
- **KTX2: GO on the merits, integration teed up as a bounded follow-up phase (not blind-landed).** Analytical spike (`cycle97-validation/ktx2-census.mjs`, `ktx2-readiness.md`, both local): the addressable set is entirely the tree impostor atlases (12 files at 2048²; dog portraits + PWA icons are DOM images, not GPU textures). Prize ~192 MB VRAM (matters most on the mobile tier, native ASTC) and ~10.6 MB net wire after the ~0.9 MB transcoder. Not landed this pass because it is a greenfield transcoder pipeline (encoder toolchain + KTX2Loader wiring across the impostor color/normal/depth load path + mobile/desktop transcode targeting) concentrated on Basis's three worst cases (alpha foliage, normal maps, depth maps), with no measured VRAM bottleneck creating urgency - an unbounded render-path landing that prod-testing validates poorly. The integration spec (UASTC-only, per-texture format choice, golden + cold-load + jitter-rail gates) is ready; greenlight it as a focused phase, and Matt's prod look validates a bounded change. Revisit sooner if an S24+ pass reports NSL VRAM pressure.
- **Held (not delegated by "I test in prod"):** NSL-as-default-world stays Rolling Hills until Matt's product call; no version bump and no launch posting (paired, Matt's voice); three r185 stays blocked (not on npm, latest 0.184.0); the rock re-bake needs a design direction (the Cycle 96 collider-parity harness is ready to gate it).

## Cycle 98 - KTX2 impostor pipeline (encode -> load) shipped to local; dead octahedral set dropped from dist; asset-weighting analysis (2026-06-14)

Matt picked KTX2 as the Cycle 98 goal from the Cycle 97 carryover, then directed "run phase 1-3" and "fold your recommendations in." Phases 1-4 of the KTX2 integration are done locally (not committed/deployed); the GPU-gated A/B (Phase 5) and the PNG-from-dist drop wait behind a concurrent GPU perf run in another repo.

- **KTX2 encode is a PNG -> KTX2 post-process via a wasm encoder, no native install.** The impostor baker (`tools/bake-tree-impostors.mjs`) shells to Pixel Forge's Kiln from gitignored `_originals/` and is local-only; the PNG atlases are committed. So KTX2 rides as a separate post-process (`tools/encode-impostors-ktx2.mjs`, `npm run encode-impostors-ktx2`) that reuses the baker's exported bake matrix (`enabledImpostorTargets`), filters to the live `latlon-hemi-y` layout, and encodes via `ktx2-encoder` (wasm; KTX-Software is NOT in winget and an admin installer was the wrong autonomous trade) with sharp as the PNG decoder. UASTC + zstd, `uastcLDRQualityLevel:3`, no mips, `isYFlip:true` to match three's flipY=true PNG convention for the compressed-texture path. 6 live atlases: 7.32 MB PNG -> 2.59 MB KTX2 (35%).
- **Runtime: one lazy KTX2Loader singleton + a shared loader helper.** `js/rendering/ktx2Loader.js` dynamic-imports `KTX2Loader` (its own 59 KiB chunk, never in main) and vendors the decode-only basis transcoder to `assets/vendor/basis/` (lazy-fetched on first decode). `loadImpostorTexture(base, suffix, colorSpace)` prefers `.ktx2`, falls back to `.png`; both load sites (`kiln-impostor-material.js` `loadKilnImpostor`, `TreePlacement.js` `loadColdImpostorAtlas`) call it. `detectSupport` is warmed at renderer-ready as a DETACHED `.then` side-effect on `SceneManager.rendererReady` - it must not alter that promise's identity or timing (a render-flow test asserts on the exact tick it resolves).
- **Bundle budgets bumped, recorded as deliberate KTX2 growth** (`tests/refactor-baseline/__fixtures__/bundle-sizes.json`): `other` 555 -> 613 KiB (the lazy KTX2Loader chunk lands in the catch-all family), `three` 605 -> 606 + `threeKB` 604 -> 605 (three classes KTX2Loader newly references), `main` 620 -> 621 (the loader wrapper). The heavy code is correctly split out; this is the wrapper plus newly-referenced three classes, per the ratchet convention a recorded, deliberate bump.
...
## Front door alignment, round three - 4 decisions locked (2026-07-26)

Cycles 117 to 120 shipped in one session and each one found something its own plan did not know. Four questions came out of them that an agent should not answer alone: one because a cycle's remaining phase turned out to be blocked by a defect nobody could see until the light was fixed, and three because they trade shipped bytes against affordances that exist for Matt's benefit rather than the player's. Answered in one sitting. These bind Cycles 121 and after.

**D33. Grass lighting comes before Home Field's evening.** Cycle 120 Phase 3 is deferred, not dropped. Making the sun honest exposed that grass is `MeshBasicNodeMaterial` taking baked indirect only, so it does not read the scene lights at all: at night it is a self-lit field over near-black ground, and grass-to-terrain goes from 10.8:1 to **204:1** on Rolling Hills. Nobody could see this while the directional was a frozen 3.456 at every hour. Home Field is wall-to-wall grass and is now the default scene **and** the entrance backdrop, so giving it an evening first would ship a glowing pasture at midnight on the first thing every player sees. The order is: grass reads the scene lights (Cycle 123), then Home Field gets its `dayNight`, then D25 closes. The payoff is already proven in a browser - the dusk lamp fires at `emissiveIntensity` 0 / 0.2696 / 2.2 across noon, golden hour and night - so the deferral costs only the scene data.

**D34. `__sdsCinema.freeFly()` and its OrbitControls chunk stay.** 20,875 bytes, and they are worth it. The `?cinematic=1` harness has found real defects in six consecutive cycles, including gate leaves hanging 1.85 m over falling ground and a HUD chevron pinned on at any gate above y=4. Cycle 119 freed 45 KB and `main` now has 20,700 bytes of headroom, so the pressure that would have justified the trade is gone. `.claude/rules/scene-and-render.md` already said this was a separate decision; it is now a settled one.

**D35. The three dev surfaces stay on production.** PlaytestNote (`?notes=1`), the wolf harness (`?wolf=1`) and grassInteractionProof ship to the live site and continue to. Excluding them at build time would save roughly 11.8 KB and would break sending a playtester a URL with a param on it, which is the entire reason they are reachable in production. Note the related finding that made this a real question rather than a free win: **gating an already-lazy import saves nothing**, because `tests/refactor-baseline/baseline.spec.ts` sums every emitted chunk with no notion of whether it is ever fetched. Only deletion or build-time exclusion moves the number, and neither is worth it here.

**D36. The impostor atlases keep their ZSTD layer.** All ten shipped `.ktx2` files carry `supercompressionScheme = 2` over Basis Universal, which is why Cycle 119 kept the 38,900-byte decoder instead of deleting it. Re-baking without ZSTD would move those bytes off the JS bundle and onto the assets. That is a genuine tradeoff (asset bytes cache, JS bytes parse) and it is also uncomfortably close to gaming a ratchet that counts one and not the other. With bundle pressure off, the ratchet keeping an honest measurement is worth more than the bytes. Not taken, and not to be re-proposed without measured deltas on both sides plus cold load.
