...
- **Per-client soft-degrade:** sessions that joined with `protocolVersion < 3` (or none) get full `gameStateUpdate` frames every broadcast interval, shape-compatible with v2 except the additive `tick` field and the frame-level `v` reading 3 (no client reads frame-level `v`). No refusal; `SURVIVAL_MIN_PROTOCOL_VERSION` stays 2.
