...
> **SCAFFOLD STUB — Goal + Phases not yet authored.** Fill these in at `/cycle-start`. Candidate scope below.
...
**Candidate scope (from Cycle 51 carryover — confirm/revise before starting):** a lighter UI cleanup cycle that lands the two Cycle 51 deferrals plus any post-review fixes, without committing to the larger Pixel Forge asset program yet:

- **In-engine dissolve reveal.** Cycle 51 P11 shipped the verified CSS crossfade reveal; finish Q4's original in-engine WebGPU dissolve as the scene-reveal. Higher-risk (touches the boot-reveal path) — research a clean hook before committing.
- **`ExtremeTuningPanel` `.tsx` migration.** The last `createElement` HUD holdout (dev-only tuning panel) — migrate to `.tsx` for parity with the rest of the HUD.
- **Post-review fixes** from Matt's live review of the Cycle 51 pastoral finish (entrance, HUD, joystick, loading, info menu).

(Not in scope here: the Pixel Forge bespoke-asset program — dog-portrait avatars, in-world props — which is a separate cycle. And the security audit roadmap / P-SEC-1, also its own cycle. See NEXT_SESSION "Program threads".)
