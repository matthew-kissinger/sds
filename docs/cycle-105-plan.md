...
Status - open 2026-06-25. `cycle105-validation/grass-ground-redesign-brief.md` records the current grass contract, r185 `positionGeometry` repair, clean dog/sheep interaction proof, prototype directions, and acceptance gates. Matt likes the opt-in `sds-hybrid-v1` candidate visually, and its Rolling Hills perf/wiring/build slice is validated: focused material tests, full `npm test`, production build, bundle ratchet, and diff-check pass. Current evidence: it reduces Rolling Hills grass triangle cost by roughly 55-60% in grass-only proof and full-scene renderer triangles by roughly 33-43%; live gameplay captures report one hybrid ground-contact draw call with 76 instances for dog plus 75 sheep. The recorded bundle cost is `chunkBudgetsKiB.other` `676` -> `683` KiB for the opt-in grass profile, ground-contact overlay, and visual proof hooks. Cross-scene still proof now compares default versus `sds-hybrid-v1` across Home Field, Rolling Hills, Open Country, and NSL at follow-close and classic-max poses, with visible grass triangle estimates down about 51-58% and the hybrid contact overlay staying at one draw call. Cross-scene WebGPU dog-sprint motion proof also passes in all four scenes with zero >50ms spikes. The grass redesign still needs Matt PC review of `cycle105-validation/grass/hybrid-cross-scene-contact-sheet.png` and an explicit accept, tune, or compare-cleaned-current decision before it can close. `cycle105-validation/foliage-fastpan-lod-report.md` records first-pass WebGPU motion proof across Home Field, Rolling Hills, Open Country, and NSL, then attributes the initial Rolling Hills/NSL spike reports to harness/cold-readiness artifacts rather than steady-state LOD cost: rAF-primed Rolling Hills passed, NSL passed with `--waitFoliage=1` and with a longer warmup, and jitter driven/idle probes stayed clean in both scenes. The same report records the measured `chunkBudgetsKiB.main` `624` -> `625` KiB ratchet for the NSL tree LOD source-matrix fix, WebGPU storage-backed canopy shadow caster, farmhouse shadow policy, and cold/far readiness diagnostics. It also records the post-fix NSL proof that trees now reach LOD0 near camera distance, the farmhouse no longer casts the monolithic square shadow, the authored NSL gate GLB is live without the old procedural overlap while still opening/closing, the accepted fence GLB kit now batches posts and rails through terrain-aware instanced segment meshes, and the current entrance flow is healthy in Chromium/Firefox/WebKit/MP release e2e after stale MP helpers were updated to `Play online` and the entrance dog picker. A WebGPU steady-state contact sheet now exists at `cycle105-validation/foliage-fastpan/contact-sheet/lod-handoff-contact-sheet.png`; remaining LOD work is Matt visual approval or targeted density/handoff tuning from that proof.
...
Status - generated, previewed, and partially integrated 2026-06-25. The first one-off farmhouse generations are rejected/reference-only because they do not establish a coherent SDS asset-pack direction. `cycle105-validation/homestead-playfield-pack-report.md` records the proper Kiln Packs path, generated pack evidence, no-ground refinements, live farmhouse approval, scene-fit placement proof, local validation, and the bundle verdict. The Kiln pack is `sds-homestead-playfield-pack-v1`, tag `sds-homestead-playfield`, status `complete`, with `paletteId=sds-pastoral-survival-v1` at pack and item level. All 13 outputs returned `ok` across farmhouse variants, utility shed, hay bales, trough, crate stack, barrel/rope, log/stump, stone markers, wildflower/weed clumps, and blank signpost. Matt approved `01-farmhouse-a` in the actual NSL scene, so it is integrated locally as `assets/models/Farm house.glb`; the runtime keeps farmhouse casting disabled until a dedicated proxy-shadow pass is approved. Candidates 10, 11, and 12 have no-ground refined rebakes to remove dirt/soil/lichen base pads. The current scene-fit diagnostic sweep mounts packyard props in Home Field and NSL, and only sparse natural accents in Rolling Hills and Open Country. `02-farmhouse-b` is reserve comparison only. The local asset-review harness is dev-gated out of production; the remaining measured bundle change is a legacy `mainKB` ratchet from 627 to 628 with per-family chunk budgets unchanged. Current local gates are green for lint, typecheck, unit tests, production build, and full Playwright e2e: `npx playwright test --reporter=list` passed with 83 passed / 19 skipped after the local Worker was started on `127.0.0.1:8787`. Next gate is Matt actual-scene approval: accept, tune placement/scale/density, or reject the remaining prop and accent groups before any additional live placement integration.
...
No phase in this cycle authorizes edits to `shared/`, `tests/sim-baseline/__fixtures__/`, terrain/scatter refactor-baseline fixtures, `docs/BACKLOG.md`, `DECISIONS.md`, `.claude/commands/`, or `.claude/rules/`. Phase 2 authorizes only the measured `tests/refactor-baseline/__fixtures__/bundle-sizes.json` r185 budget update after the decision is recorded. Cycle 5 authorizes only the measured `chunkBudgetsKiB.other` update for the opt-in grass candidate after the decision is recorded in `cycle105-validation/grass-ground-redesign-brief.md`, plus the measured `chunkBudgetsKiB.main` update for the NSL tree LOD/shadow fix after the decision is recorded in `cycle105-validation/foliage-fastpan-lod-report.md`. Phase 6 authorizes the measured `mainKB` and `chunkBudgetsKiB.main` update for terrain-aware fence GLB instancing after the decision is recorded in `cycle105-validation/fence-candidate-report.md`. Cycle 6 authorizes the measured legacy `mainKB` update from 627 to 628 for the broader-pack live farmhouse and scene-fit review pass after the decision is recorded in `cycle105-validation/homestead-playfield-pack-report.md`; per-family chunk budgets stay unchanged because the asset-review harness is dev-gated out of production. If later evidence says another frozen file must change, stop and amend the plan with the fence migration story before editing.
