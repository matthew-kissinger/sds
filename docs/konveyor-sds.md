...
- There is a diagnostic-only WebGPU/TSL boot path at
  ...
  node material, relights from the normal aux layer, and samples the depth aux
  ...
  preview now renders scene-zone samples generated by a diagnostic-only seeded
  mirror of `RockPlacement` formation rules for all three shipped rock GLBs
  through native `THREE.InstancedMesh`; it does not change production
  `Math.random()` rock placement and does not wire shared obstacle state.
