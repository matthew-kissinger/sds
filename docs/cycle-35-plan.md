...
> Drafted 2026-05-10 after Cycle 34 (`mp-island-scenes`) closed. Cold-start agents: read [`../NEXT_SESSION.md`](../NEXT_SESSION.md) first, then this doc top-to-bottom. Prior cycle plans live in [`archive/cycles/`](archive/cycles/).

> **STUB.** This cycle has no goal yet. The leading carryover candidate is post-deploy verification of Cycle 34's MP island scenes (a manual playtest pass on `?scene=open-country` MP rooms to confirm the round-up → drive transition + lobby UI feel right). See [`BACKLOG.md`](BACKLOG.md) for other deferred items.
