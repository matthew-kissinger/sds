...
**Validation gates:** `npm test` 1135 pass / 8 skip / 0 fail; `npm run lint` (`eslint shared/`) clean; `npm run build` clean. Bundle ratchet 600.54 kB main / 618.78 kB three == Cycle 75 baseline. The scene + render files were temporarily edited for the measurement (pin lifted, storage flag, chunkSize) and restored byte-identical.
