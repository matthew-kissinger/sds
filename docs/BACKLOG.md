...
- **Validation gates.** `npm test` green after the deliberate `other` bundle ratchet bump `551` -> `552` KiB for the lazy bark hint chunk; `npm run lint` green; `npm run build` green; `npm run test:integration` green; `npm run test:e2e -- --project=chromium tests/e2e/smoke.spec.ts` green after the `v2.3.3` stale-timer fix. Full `npm run test:e2e` was attempted, but the local Playwright-managed Vite server died after the first Chromium foliage test and the rest failed with `ERR_CONNECTION_REFUSED`; production-preview browser probes covered this cycle's bark surfaces instead. Preview proof showed desktop `Bark Space`, bark dismissal, live sheep speeds `0.0211` / `0.0420` under the ordinary `0.1` cap, mobile `barkHintCount: 0`, and mobile `barkButtonCount: 1`.
