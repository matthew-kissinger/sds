...
> **STUB — needs slug + Goal + Phases.** Scaffolded 2026-05-09 from Cycle 30's autonomous close. Cold-start agents: read [`../NEXT_SESSION.md`](../NEXT_SESSION.md) first, then this doc top-to-bottom. Prior cycle plans live in [`archive/cycles/`](archive/cycles/).
>
> **Author hint (Matt's pickup):** [`BACKLOG.md`](BACKLOG.md) "Deferred / not blocking" carries several Cycle 31 candidates. The two that look most ready (carried from Cycle 30's stub):
>
> 1. **MP island scenes** — Rolling Hills + Open Country in multiplayer. Cycle 6's `TreePlacement` lift made this feasible; Cycle 30's heightfield-Y unification removes one source of silent disagreement between Worker + client. Remaining work is wiring obstacle bundle into Worker `GameSim` init + `obstacleAvoidance` in shared sheep/dog tick. Solo Phase 2 wiring is the template. Sim-deterministic; will need a careful sim-baseline regen story.
...
>
> Pick a slug that reflects the chosen scope, rename the file, and run `/cycle-start`.
