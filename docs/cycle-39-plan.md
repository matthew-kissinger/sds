...
> Drafted 2026-05-20 after Cycle 38 closed. Cold-start agents: read [`../NEXT_SESSION.md`](../NEXT_SESSION.md) first, then this doc top-to-bottom. Prior cycle plans live in [`archive/cycles/`](archive/cycles/).
>
> **Stub — fill in Goal + Phases before running `/cycle-start`.** Suggested seed scope from Cycle 38 carryover: octahedral tree-impostor sidecar v2 + Kiln material projection, OR a paired mobile-readiness cycle for Phases 5/6 (Android matrix, iOS Safari canary, OC MP playtest).
