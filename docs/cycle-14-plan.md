...
| 3 — Trees | `_patchTreeWindMaterial()` + `_setupTreeWind()` on `TerrainBuilder` — `onBeforeCompile` leaf-wind patch on existing tree GLBs. Shared uniforms; idempotent. | EZ-Tree build-time bake (`tools/bake-trees.mjs` + `tools/bake-trees/bake.html`) generates 3 stylized GLBs into `assets/models/trees/`. `@three.ez/instanced-mesh@0.3.15` upgrades both near + far InstancedMesh sites with per-instance frustum culling. | ✅ **fully shipped** (commits `ec0b902` shader + `9f025f8` asset bake + InstancedMesh2 refactor). Full LOD-pool unification (per-instance dynamic switch full mesh → impostor) deferred to Cycle 15 — needs trunk-only/leaves-only impostor authoring. |
...
**Cycle stats:** 158/158 vitest pass (was 149; +9 from new mesh-Y spec). Production build clean (main 808 KB / 239 KB gzip — +57 KB from `@three.ez/instanced-mesh` + bvh.js, +7 KB from shader patches). Sim-baseline byte-identical (the whole point of routing visuals through `meshSampleY` while sim keeps `sample()`).

**What an agent picking up the remaining asset work needs to know:**

- **Trees ✅ done.** `npm run bake-trees` regenerates the 3 hero GLBs from seeded recipes in `tools/bake-trees.mjs`. Re-run after editing recipes; commit the result. The `@three.ez/instanced-mesh@0.3.15` `InstancedMesh2` upgrade applies to both near and far tree paths.
- **Rocks: Quaternius MegaKit (still Matt-gated).** Manual download from [quaternius.com/packs/stylizednaturemegakit.html](https://quaternius.com/packs/stylizednaturemegakit.html); drop into `assets/models/rocks/`. The same pack feeds the Phase 4 ScatterSystem (mushrooms, flowers, pebbles, sticks).
...
- Tree placement in [`shared/TreePlacement.js`](../shared/TreePlacement.js) is the contract; only the GLB *targets* in [`js/TerrainBuilder.js`](../js/TerrainBuilder.js) `modelPaths.trees` (already updated to `assets/models/trees/`) and `modelPaths.rocks` (still pointing at the old `Resource_Rock_*.glb`) need updating.
...
- `_treeWind.uWindStrength.value` defaults to 0.6 desktop / 0 mobile. Tune in browser if leaves look too active or too still.
...
- ScatterSystem authoring guide: use `InstancedMesh2` for consistency with the tree path. Constructor mirrors GrassSystem; Poisson sample radius ~0.4m, capped ~2k; per-frame culling beyond ~40m camera radius; subtle wind sway on flowers (re-use the leaf-wind shader patch math at 0.3× amplitude).
