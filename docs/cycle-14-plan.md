...
**Known visual issues remaining (must address before Phase 5 hero cards):**

Captured 2026-05-03 via [`tools/probe.mjs`](../tools/probe.mjs) + Matt's eyeball review of the deployed build:

1. **Trees still need more leaves.** The brown-bark + relaxed-children hotfix lifted the trees substantially over the white-skeleton state but at the typical play-camera distance they still read as "moderately leaved" rather than "lush canopy." Visible branch structure on several mid-ground trees in the Rolling Hills probe. Iteration: bump `leaves.count` further (40+), bump `leaves.size`, raise `branch.children` toward EZ-Tree default `{ 0: 7, 1: 5, 2: 3 }` for more leaf-cluster anchor points.
2. **Some trees float above terrain.** Spotted in Rolling Hills + Open Country probes. Diagnose: (a) far-tree cross-billboard quads on slopes (impostor's flat quad placed at terrainY may extend above ground when slope is steep), (b) horizon-zone trees on the flat skirt — `meshSampleY` should return 0 there but heightfield smoothstep falloff might produce slightly negative values, (c) bake-harness `tree.position.y = -bbox.min.y * s` lift surviving GLTFExporter into non-zero `modelBboxMinY` after the load-time bake. Add a debug overlay that draws each tree's `placementY` vs visible terrain Y to catch the offending zone(s).
3. **Rocks look like broken mesh shards, not rocks.** Quaternius MegaKit `Rock_Medium_1/2/3.gltf` rendering as faceted geometric fragments. Possible causes: 128px diffuse texture downsample too aggressive, flat-shading-incompatible vertex normals, or 0.2m `ROCK_NATIVE_HEIGHT` normalization being too small for the silhouette to read. **Decision needed**: source different CC0 stylized rocks (Kenney Nature Kit, KayKit Forest, hand-picked Poly Pizza items) OR commission custom rocks in pixel-forge. The rim-light shader patch + ScatterSystem will pick up any new rock GLBs automatically the moment they land at `assets/models/rocks/`.
