...
## The leaderboard: D12's premise is false, and the choice is Matt's
...
D12's own escape clause fires here: *"If a real score has appeared by then, archive as all-time instead and start a new board for the gate objective."* **Archiving and resetting are different pieces of work and the choice is Matt's.**

**This cycle deletes nothing.** Phase 7 exports the affected rows to a validation artifact and stops. If a reset or archive ever ships as SQL it is a **new** `worker/migrations/0011_*.sql` (`0001` to `0010` are immutable), and it is scoped by explicit row id, never by `scene_id`.

## Open questions for Matt, surfaced not answered

1. **Leaderboard: archive as all-time, or reset, or accept incomparable history?** See above. Blocking for Phase 7 only.
2. **Rolling Hills competitive and timed.** `shared/scenes/rolling-hills.js:113` advertises both, but `shared/CompetitiveLayout.js:16-30` hardcodes the layout to Home Field geometry regardless of scene. Keep a corral for competitive, drop Rolling Hills to cooperative-only, or ship N pastures. **Must be decided before the corral delete lands**, since the delete removes the fallback. Phase 2 assumes cooperative-only unless told otherwise, and says so in the diff.
...
**Acceptance (EARS):** When Phase 1 ships, then all 11 sim-baseline fixtures shall be byte-identical, since no scene data has moved. When a rect pasture is constructed, then every barrier method shall behave as it does for a square.
...
## Phase 7 - Leaderboard disposition, read-only (~1hr)

1. Export the affected rows to `cycle117-validation/`. Report count and player-authored status.
2. **Stop there.** Delete nothing. Surface the archive-versus-reset choice.

**Acceptance (EARS):** When Phase 7 ships, then the affected rows shall be archived to a validation artifact and no row shall have been deleted or modified.
...
1. **No leaderboard delete or update.** Read-only, then stop. Real player rows exist; the choice is Matt's.
...
- [ ] When Phase 1 ships, then all 11 sim-baseline fixtures shall be byte-identical.
...
- [ ] When Phase 3 ships, then `island-boundary-rh-60hz.json` shall be byte-identical and nine other fixtures shall be untouched.
...
- [ ] When Phase 7 ships, then the affected leaderboard rows shall be archived to a validation artifact and no row shall have been deleted or modified.
