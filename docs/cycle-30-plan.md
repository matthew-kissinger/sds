...
> **STUB — needs slug + Goal + Phases.** Scaffolded 2026-05-09 from Cycle 29's autonomous close. Cold-start agents: read [`../NEXT_SESSION.md`](../NEXT_SESSION.md) first, then this doc top-to-bottom. Prior cycle plans live in [`archive/cycles/`](archive/cycles/).
>
> **Author hint (Matt's pickup):** [`BACKLOG.md`](BACKLOG.md) "Deferred / not blocking" carries several Cycle 30 candidates. The two that look most ready:
>
> 1. **MP island scenes** — Rolling Hills + Open Country in multiplayer. Cycle 6's `TreePlacement` lift made this feasible; remaining work is Worker GameSim init + obstacleAvoidance in shared sheep/dog tick. Solo Phase 2 wiring is the template.
> 2. **Heightfield Y full unification** — bake `displacedHeights` mirroring the terrain mesh vertex grid; replace the +0.05m defensive lift in `Heightfield.surfaceY`. Sim-deterministic; touches a frozen file with a migration story.
>
> Pick a slug that reflects the chosen scope, rename this file, and run `/cycle-start`.
