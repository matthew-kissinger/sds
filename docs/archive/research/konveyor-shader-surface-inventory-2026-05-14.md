...
directly when no override factory is supplied. The default `ShaderMaterial`
path and CPU LUT authority remain untouched when no factory is supplied.
...
2. Keep sky/fog production wiring behind parity evidence for preset screenshots
   ...
   reaches `Atmosphere` and `HosekWilkieSky`. The same atmosphere seam now
   reaches production `CloudLayer` with update controls. Both paths are still
   factory supplied and do not replace the default WebGL sky or cloud material
   by themselves.
