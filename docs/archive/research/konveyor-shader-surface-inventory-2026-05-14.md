...
albedo atlas, samples one atlas tile in a WebGPU node material, and records the
normal/depth aux layers plus tile blend, relighting, parallax, depth discard,
...
| 9 | Kiln tree impostors | `js/kiln-impostor-material.js` | Atlas-sampled tree impostors with relighting, alpha hash, fog, parallax/depth scaffolding. | Very high. It is asset-pipeline coupled and must match LOD0 color. | Diagnostic one-species TSL island now fetches the `tree1` sidecar and albedo atlas, samples one tile with alpha/fog, and records normal/depth aux layers. Barycentric tile blend, normal relighting, parallax, depth discard, production LOD, and LOD color matching remain deferred. | LOD color-match artifacts, tree visibility, perf, screenshots. |
...
4. Defer production terrain, production blade grass, production sheep
   ...
   island proves sidecar/atlas fetch and a WebGPU texture-sampling route only;
   it does not yet prove barycentric tile blending, normal/depth relighting,
   parallax, depth-discard ghost suppression, production LOD wiring, or color
   parity against LOD0.
