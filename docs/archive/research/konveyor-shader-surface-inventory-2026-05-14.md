...
tree-leaf node-material candidate now lives in
`js/world/konveyorTreeLeafNodeMaterial.js`, and the material adapter spec
proves the flagged production seam can route `leaves` through it while default
WebGL tree wind and occluder patching remain untouched. A diagnostic anime-water
...
|---|---|---|---|---|
| Tree wind plus occluder fade | `js/world/shaderPatches.js`, `js/shaders/OccluderFadePatch.js` | Tree GLB leaf `MeshStandardMaterial` instances. | Adds wind sway, alpha hash, and camera-to-dog dither fade. | Diagnostic leaf TSL material now proves wind, alpha-hash posture, and occluder fade inputs. The reusable WebGPU tree-leaf node-material candidate now lives in `js/world/konveyorTreeLeafNodeMaterial.js`, and the material adapter spec proves the flagged production seam can route `leaves` through it. Default WebGL still uses the existing `onBeforeCompile` tree wind and occluder patch path. |
...
- The production-side adapter in `js/world/konveyorMaterialAdapter.js` reuses
  ...
  materials stay untouched. The reusable WebGPU tree-leaf node-material
  candidate now lives in `js/world/konveyorTreeLeafNodeMaterial.js`, and the
  adapter seam spec proves the flagged `leaves` factory can route through it
  while default WebGL tree wind and occluder patching stay untouched.
