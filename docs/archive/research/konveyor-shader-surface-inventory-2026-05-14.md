...
later seams (`mainKB=575`, `threeKB=603`).
...
- `SunBillboard` itself is now loaded through a scene-coupled dynamic import.
  ...
  and keeps the current default bundle at `mainKB=575` / `threeKB=603`.
...
2. Keep sky/fog production wiring behind parity evidence for preset screenshots
   ...
   reaches `Atmosphere` and `HosekWilkieSky`, but it is still factory supplied
   and does not replace the default WebGL sky material by itself.
