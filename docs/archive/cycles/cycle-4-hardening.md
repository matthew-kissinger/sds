...
| Bug | Fix |
|---|---|
| Camera Follow look-at jittered on PC at high refresh rates (mobile felt smooth) | `_updateFollow` smoothed camera **position** with τ=0.35s but used **raw** `Math.atan2(velocity.x, velocity.z)` for the look-ahead direction. PC keyboard input + physics micro-noise (fence contact, slope, boundary nudges) made that raw atan2 jitter at 144Hz while the position lerp was smooth. Mobile's analog joystick produces a steady velocity vector → no jitter. Added `followAimYaw` smoothed at τ=0.08s for the look-ahead; position lag preserved. [CameraController.js](../js/CameraController.js) |
| Mobile camera-mode chip overlapped MobileHUD chip (both centered at top) | `CameraModeIndicator` was top-center on every platform. Mobile MobileHUD lives top-center too (pause + sheep + timer + stamina). Top-right collides with the zoom rail in landscape and a wide MobileHUD on narrow portrait. Moved mobile to top-left (SheepCounter is desktop-only and folded into MobileHUD on mobile, so the slot is empty). Desktop unchanged. [CameraModeIndicator.js](../js/components/GameHUD/CameraModeIndicator.js) |
| Terrain edge visible at max zoom-out as a faint cutoff line | Plane was 2400m / 1600m → perpendicular edge ~40% fogged at FogExp2 density 0.0006. User-aligned constraint: don't increase fog. Grew plane to **4000m / 3200m** (segments 384 / 256, quad ~10.4m / 12.5m). Heightfield content unchanged (confined to inner ±200m via existing radial falloff); new outer area is longer flat skirt fading into existing fog. Edge now ~76% / ~69% fogged. Camera far plane bumped 2800/1800 → **4500/3700** to cover the new diagonals (~2828m / ~2263m). [TerrainBuilder.js createTerrain](../js/TerrainBuilder.js), [SceneManager.js](../js/SceneManager.js) |
| Impostor trees + rocks floated at distance (visible in Follow camera) | `heightfield.sample()` clamps to edge values past the heightfield's `worldSize` (typically ±200m), but the terrain mesh applies a smoothstep falloff to 0 over the last 20m. Trees in midField/farField/horizon zones (up to ±800m) and rocks in the same zones sampled the clamped edge height → sat floating above the flat skirt at the heightfield's edge value. Added `_groundY(x, z)` helper that mirrors the terrain falloff exactly; routed tree, rock, and farmhouse placement through it. Inside ±180m: identical to before. 180–200m: smooth ramp matching terrain. Past 200m: y=0, sitting on the flat skirt. [TerrainBuilder.js _groundY + createTrees + addEnvironmentDetails + addFarmHouse](../js/TerrainBuilder.js) |
| Sprint-exhaust stuttered camera + dog animation (especially mid-turn or on diagonals) | `Sheepdog.move` hard-clamped `velocity.normalize().multiply(currentMaxSpeed)` from 25→15 in one frame the moment stamina hit 0. That popped the camera `speedNorm` (look-ahead distance halved instantly), let the position lerp surge forward as the dog decelerated, and forced animation crossfade SPRINTING→RUNNING faster than legs could blend. Replaced with `smoothMaxSpeed` that snaps on the way UP (sprint press stays responsive) and eases with τ=0.2s on the way DOWN. Safety clamp uses the smoothed cap. Velocity now decays naturally via the existing acceleration logic over ~75ms; animation passes cleanly through SPRINTING→RUNNING and camera tracks smoothly. [Sheepdog.js move](../js/Sheepdog.js) |
