...
3. **Renderer sync on MP join.** As of 2026-04-24 end of session, MP `sceneId` flows end-to-end on the sim side (host's URL-param scene is sent in `createRoom`, validated by the Worker, stored on the room, read by `GameSim`). What's *not* done: when a joiner with a different client-side scene joins a room, their client keeps rendering the local scene. Host renders Rolling Hills; joiner sees Home Field terrain but plays on Rolling Hills bounds. Fix wants either (a) a pre-join scene-mismatch prompt that redirects to `?scene=<room_id>` + auto-rejoin, or (b) making client scene reactive to room state at runtime. (a) is simpler; (b) is correct. Track 2 scene-first restructure should decide.
