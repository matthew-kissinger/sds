...
(Intake candidates from Cycle 91 close, see BACKLOG carryover: the NSL 1%-low frame floor - the pill gate read 70.5 in one battery and 54.2 in the next on identical code, with an intermittent ~146-160ms stall and heap-drop hitch fraction 0.37-0.44 pointing at GC; chase the variance to a cause, fix the stall, re-run the pill gate on a controlled box state. Also pending: P8 lighting items (keyframed hemisphere ambient, sky-dome render-order A/B), rock re-bake behind a collider-parity harness, KTX2, golden re-capture after Matt approves the Cycle 91 tree remake.)
