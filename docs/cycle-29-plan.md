...
>
> **STUB — needs Goal + Phases filled in. Run `/cycle-start` after that.** Author hint: the [`BACKLOG.md`](BACKLOG.md) "Deferred" entry describes the `GameState.js` decomposition target (1,313 → ≤ 800 LOC, mode dispatch → data-driven config). Cycle 28's refactor-baseline harness pattern in [`tests/refactor-baseline/`](../tests/refactor-baseline/) is the template for the characterization phase — capture mode-dispatch + win-condition behavior under goldens BEFORE extracting.
