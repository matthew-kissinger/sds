# Next Session — Cycle 14 (`visuals-foundation`) Phases 1–3 fully shipped; Phase 4 partial; rocks + ScatterSystem + hero cards are Matt-gated

> Updated 2026-05-03 (autonomous Cycle 14 close pass: shader half + tree asset bake both landed in a single session). **Active plan: [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md).** What shipped: heightfield-Y unification, grass gust-envelope wind + sun-aligned fake-SSS, leaf-wind shader patch on trees, fresnel rim-light on rocks, full **Phase 3 close** via EZ-Tree build-time bake (`npm run bake-trees`) + InstancedMesh2 per-instance frustum culling. What's Matt-gated: Quaternius MegaKit rock GLB swap (manual download), new `js/ScatterSystem.js` (entangled with rock pack), hero-card re-renders (browser playtest required), v1.1.0 tag. **158/158 vitest pass; sim-baseline byte-identical; production build clean (main 808 KB / 239 KB gzip — +57 KB from `@three.ez/instanced-mesh` + bvh.js).** Last closed cycle: [`docs/archive/cycles/cycle-12-plan.md`](docs/archive/cycles/cycle-12-plan.md). Cold-start agents: read this page top-to-bottom, then [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md), then the three research dossiers in `docs/research-*-2026-05.md`. Earlier cycles: [`docs/archive/cycles/cycle-11-plan.md`](docs/archive/cycles/cycle-11-plan.md), [`docs/archive/cycles/cycle-10-plan.md`](docs/archive/cycles/cycle-10-plan.md).
...
- **Phase 3 — Trees ✅ FULLY SHIPPED** (commits `ec0b902` shader, `4a98245` plan pivot, `9f025f8` LOD refactor + bake commit). Three pieces:
  - **Leaf-wind shader patch** (`ec0b902`): `_patchTreeWindMaterial()` + `_setupTreeWind()` on `TerrainBuilder` — `onBeforeCompile` that replaces `<project_vertex>` so wind is applied AFTER per-tree instance rotation. Wind weight = `smoothstep(0.25, 1.0, posY01)²` (vertical fraction up the tree from bbox min to max). Same gust-envelope + 2-octave sway math as grass, lower amplitude (0.18 multiplier). Direction synced from `grassSystem` so grass and trees agree on wind. WeakSet guard makes the patch idempotent across scene swaps.
  - **EZ-Tree build-time bake**: `tools/bake-trees.mjs` (Node driver) + `tools/bake-trees/bake.html` (Playwright harness page) generate stylized cozy-game trees from EZ-Tree v1.1.0 at build time. Run `npm run bake-trees` then `npm run compress-glbs`. Three recipes seeded for byte-stable output: tree1 (Aspen Medium, slim vertical), tree2 (Oak Medium, broad canopy), pine (Pine Medium, conifer fronds). Bark de-textured (saves ~1.5MB per tree); branch.children cut to 4/2/0 (~1500 tris target); leaf alpha PNG downsampled 1024² → 256² (~16x size cut). Final: tree1 81 KB / 1092 tris, tree2 112 KB / 1744 tris, pine 91 KB / 296 tris — total **284 KB across 3 GLBs** (was ~30 KB across the old low-poly bunch, but visibly much higher fidelity). Located at [`assets/models/trees/`](assets/models/trees/); old `Resource_Tree*.glb` deleted.
  - **InstancedMesh2 per-instance frustum culling**: dropped `THREE.InstancedMesh` for `@three.ez/instanced-mesh@0.3.15` on both the near (full GLB mesh) and far (cross-billboard impostor) tree paths. Out-of-frame instances skip vertex shader execution — meaningful win at SDS's tree counts (50–200 per scene) and the ~270° impostor ring. Drop-in replacement; the leaf-wind patch survives untouched (uses the same `instanceMatrix` attribute). Bundle: +57 KB raw / +17 KB gzip from `@three.ez/instanced-mesh` + `bvh.js`. Full LOD-pool unification (per-instance switch from full mesh to impostor) deferred to Cycle 15 — needs trunk-only and leaves-only impostor authoring since EZ-Tree splits each tree into trunk + leaves children with separate materials.
...
### Carry-forward work (Matt-gated remainder)

Phases 1–3 are fully closed. Phase 4 is partially closed (rim-light shader on existing rocks). Three threads remain to fully close Cycle 14, all needing either external asset downloads or browser playtest:

1. **Phase 4 rocks — Matt-gated.** Download [Quaternius Stylized Nature MegaKit](https://quaternius.com/packs/stylizednaturemegakit.html) (CC0); cherry-pick 4–6 hero rocks; drop into `assets/models/rocks/`; run `npm run compress-glbs`; update `modelPaths.rocks` in [`js/TerrainBuilder.js`](js/TerrainBuilder.js). The fresnel rim-light shader patch already shipped (commit `42c9f63`) walks every rock child material at GLB load time — new MegaKit assets pick up the rim-light automatically with no shader changes.
2. **Phase 4 ScatterSystem — Matt-gated on the rocks pack.** Same MegaKit download covers pebbles/sticks/mushrooms/wildflowers. Author [`js/ScatterSystem.js`](js/ScatterSystem.js) sibling to GrassSystem. Constructor `(scene, isMobile, sceneDef, heightfield, boundary)`. Internals: Poisson-disk sample on the heightfield XZ plane (radius ~0.4m, capped ~2k samples); render 4–5 prop variants via `InstancedMesh2` (consistent with the upgraded tree path); ~70% pebbles/sticks, ~20% small flora, ~10% punctuation clusters; cull beyond ~40m camera radius. Yellow-dandelion oversampling on a subset of points (5–8 in 1.5m) for Ghibli eye-anchor clusters. Wire after GrassSystem in scene init; add to `disposeScene` teardown.
3. **Phase 5 hero cards — Matt-gated.** Open in browser → start Solo Extreme → `await __sdsCinema.freeFly()` → pose → `snapshotPose()` → paste into [`tools/cinematic/shot-list.mjs`](tools/cinematic/shot-list.mjs) → `npm run cinema --shot=og-rh-sunset` (and the two siblings). Render the four cinematic videos. Tag v1.1.0.
