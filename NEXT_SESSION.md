...
**Cycle 94 closed 2026-06-12** (plan archived at `docs/archive/cycles/cycle-94-plan.md`). This was a narrow bark hotfix from Matt's mobile/PC playtest feedback, shipped as `v2.3.1`: bark now steers sheep through bounded acceleration, not direct velocity injection, and desktop play has a dismissible `Bark Space` cue while mobile keeps the visible bark button. Cycle 93 still remains the next planned visual-queue cycle unless Matt retargets it.
