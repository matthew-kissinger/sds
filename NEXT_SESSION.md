...
> Updated 2026-05-05 (post-Cycle-23 close shipped as `v1.4.0`, plus three commissioned research docs to shape Cycle 24 scope). Cycle 23 closed end-to-end autonomous: pitch-aware desat, scene-fog wiring, kiln impostor pitch-tilt, default Follow cam, **camera-to-dog occluder fade** (novel game-dev trick: leaves dither when blocking line of sight), stamina sprint-exit lock-out, OC HUD vertical-stack, HardwareTier service, grass T4 meadow-quad LOD, MP cap to 5000 + mobile-guest gate, cinematic-flag invite-strip. All on `main` plus 8 phase tags + `v1.4.0`. **Active plan: [`docs/cycle-24-plan.md`](docs/cycle-24-plan.md) — research-shaped scope.** Six phases: lobby lifecycle tests, in-game + scene-swap MP coverage, 15s reconnect grace window (Colyseus default; Cycle 23 audit found `RoomDO` evicts immediately — phone-backgrounding drops sessions), MP dog selection wiring + display verification (per Matt's mid-cycle directive: each user must see correct dog mesh for every other user), optional render-texture grass-trample + WebGPU renderer feature-flag spikes, ship `v1.5.0`. Five v1.4.0 playtest items (Classic-overhead trees, sprint exit, OC HUD, MP modes, tree tris) deferred to end of Cycle 24 close per Matt's directive. Research docs at [`docs/cycle-24-research-mp-testing.md`](docs/cycle-24-research-mp-testing.md), [`docs/cycle-24-research-foliage.md`](docs/cycle-24-research-foliage.md), [`docs/cycle-24-research-batched-webgpu.md`](docs/cycle-24-research-batched-webgpu.md).
