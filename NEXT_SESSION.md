# Next Session — Cycle 14 (`visuals-foundation`) Phases 1–4 SHIPPED + DEPLOYED; ready for Matt's visual review at sheepdogsim.com

> Updated 2026-05-03 (autonomous Cycle 14 close pass: shader half + tree asset bake + Quaternius rock + ScatterSystem + integration-fix audit all landed in a single session, deployed to production). **Active plan: [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md).** What shipped: heightfield-Y unification (Phase 1), grass gust-envelope wind + sun-aligned fake-SSS (Phase 2), EZ-Tree build-time bake + leaf-wind shader + InstancedMesh2 per-instance culling (Phase 3), Quaternius MegaKit rocks with fresnel rim-light + new [`js/ScatterSystem.js`](js/ScatterSystem.js) for the alive-meadow detail layer (Phase 4), plus a load-time pivot/scale audit that fixed Quaternius asset oversize + centroid-pivot issues before they reached the browser. **What's Matt-gated:** Phase 5 hero-card re-renders (browser playtest + `freeFly` posing + `npm run cinema` × 7) → v1.1.0 tag. **First step: visit [sheepdogsim.com](https://sheepdogsim.com) and walk Field / Rolling Hills / Open Country at noon + sunset.** Tuning knobs to flag if anything looks off: `_treeWind.uWindStrength` (0.6 default), `_rockShader.uRimStrength` (0.35), `ROCK_NATIVE_HEIGHT` (0.2m at load time), per-variant `targetHeight` in [`js/ScatterSystem.js`](js/ScatterSystem.js). **158/158 vitest pass; sim-baseline byte-identical; production build clean (main 815 KB / 241 KB gzip).** Last closed cycle: [`docs/archive/cycles/cycle-12-plan.md`](docs/archive/cycles/cycle-12-plan.md). Cold-start agents: read this page top-to-bottom, then [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md), then the three research dossiers in `docs/research-*-2026-05.md`. Earlier cycles: [`docs/archive/cycles/cycle-11-plan.md`](docs/archive/cycles/cycle-11-plan.md), [`docs/archive/cycles/cycle-10-plan.md`](docs/archive/cycles/cycle-10-plan.md).
...
### Carry-forward work (Phase 5 only, Matt-gated)

Phases 1–4 are fully closed. Only the hero-card re-render + v1.1.0 tag remain:

1. **Phase 5 hero cards — Matt-gated.** Open in browser → start Solo Extreme → `await __sdsCinema.freeFly()` → pose → `snapshotPose()` → paste into [`tools/cinematic/shot-list.mjs`](tools/cinematic/shot-list.mjs) → `npm run cinema --shot=og-rh-sunset` (and the two siblings). Render the four cinematic videos. Tag v1.1.0. The polished world is now ready for hero-card framing — eyeball the EZ-Tree silhouettes + ScatterSystem coverage during the first playtest before pinning shots.
