...
> **Updated:** 2026-06-25
> **For:** Cycle 105
> **Pickup priority:** Continue larger Cycle 2 in [`docs/cycle-105-plan.md`](docs/cycle-105-plan.md): generate a second fence candidate that reads as repeatable fence spans, because the first staged Kiln candidate was visually rejected.

## Goal

Cycle 105 is the kickoff for the larger SDS r185 + Kiln Asset Renewal program. The program goal is to move SDS onto the current Three.js release and rebuild the non-core asset set around a coherent SDS Kiln palette, measured runtime budgets, and Matt-approved PC visual/playtest gates while keeping sheep and dog stable. Cycle 1 has the r185 dependency/render foundation locally implemented. Cycle 2 now has a palette, locked local Kiln pack, fence spec, and one rejected staged fence candidate. The next chip-away target is a second fence candidate with a fence-first silhouette before any live fence replacement.
...
- **Phase 5 - Fence candidate bake and staging (autonomous):** shipped; evidence exists at `cycle105-validation/fence-candidate-report.md`; ignored candidate files live under `cycle105-validation/fence-candidates/`; the first candidate passed technical staging but failed visual review.
- **Phase 6 - Fence visual approval and integration (paired):** rejected for the first candidate; Matt said it looked like a post and gate rather than a fence. The live runtime fence remains unchanged. The next pass must generate a fence-first candidate with repeatable straight spans and connected preview evidence.
...
These artifacts are rejected evidence, not production candidates.
...
- The first fence candidate is rejected because it reads as a post/gate prop, not a fence. The next candidate must present connected, repeatable fence spans before review.
