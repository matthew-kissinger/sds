# Next Session — Cycle 14 (`visuals-foundation`) Phases 1–4 SHIPPED + visual-issues followup landed; Phase 5 hero cards Matt-gated

> Updated 2026-05-03 (visual-issues followup pass: denser tree bake + procedural rocks + trunk-only ground reference). **Active plan: [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md).** Live on [sheepdogsim.com](https://sheepdogsim.com). What shipped: heightfield-Y unification (P1), grass gust-envelope wind + sun-aligned fake-SSS (P2), EZ-Tree build-time bake + leaf-wind shader + InstancedMesh2 per-instance culling (P3), procedural rocks + new [`js/ScatterSystem.js`](js/ScatterSystem.js) (P4). **Visual-issues followup (this pass)** addressed all three flagged issues: (1) **Tree leaf density bumped** — `STYLIZED_BARK.leaves.count` 28→48 (oak 64), `branch.children` `{0:6,1:4,2:2}`→`{0:8,1:5,2:3}`, pine-only `{0:14,1:6,2:3}` for canopy volume; leaf-alpha texture 256²→384² for mid-distance crispness. Trees now 2.2 MB total (was 899 KB). (2) **Floating trees fixed** at load-time — `modelBaseYOffset` now uses **trunk-mesh-only** bbox in [TerrainBuilder.js](js/TerrainBuilder.js) so drooping leaves no longer drive ground reference (drooping leaves below trunk-base were lifting trees by `lowest-leaf × scale ≈ 0.5–1m hover` at scale 15×). (3) **Rocks replaced** with deterministic procedural icosa+simplex-noise bake at [`tools/bake-rocks.mjs`](tools/bake-rocks.mjs) + [`tools/bake-rocks/bake.html`](tools/bake-rocks/bake.html) — three recipes (small/cobble/boulder) with two-octave 3D simplex displacement, per-rock non-uniform xyz scale for silhouette variety, baked vertex-color AO (Y-position gradient), MeshStandardMaterial flat-shaded. ~33 KB total (was 137 KB Quaternius). New `npm run bake-rocks` script. **What's Matt-gated:** verify the fixes look right on prod deploy → Phase 5 hero-card re-renders → v1.1.0 tag. **158/158 vitest pass; sim-baseline byte-identical; production build clean (main 815 KB / 241 KB gzip); CI Mac Safari smoke green.** Last closed cycle: [`docs/archive/cycles/cycle-12-plan.md`](docs/archive/cycles/cycle-12-plan.md). Cold-start agents: read this page top-to-bottom, then [`docs/cycle-14-plan.md`](docs/cycle-14-plan.md), then the three research dossiers in `docs/research-*-2026-05.md`. Earlier cycles: [`docs/archive/cycles/cycle-11-plan.md`](docs/archive/cycles/cycle-11-plan.md), [`docs/archive/cycles/cycle-10-plan.md`](docs/archive/cycles/cycle-10-plan.md).
...
### Visual-issues followup (2026-05-03 — what landed this pass)

All three flagged issues addressed in one pass; full plan at [`C:\Users\Mattm\.claude\plans\lovely-wibbling-acorn.md`](C:\Users\Mattm\.claude\plans\lovely-wibbling-acorn.md).

1. **Tree leaf density.** Re-baked with denser config in [`tools/bake-trees.mjs`](tools/bake-trees.mjs):
   - Shared `STYLIZED_BARK.leaves.count`: 28 → **48** (+71%); oak override → **64**.
   - `leaves.sizeVariance`: 0.5 → 0.55.
   - Shared `branch.children`: `{0:6, 1:4, 2:2}` → **`{0:8, 1:5, 2:3}`** (+33% silhouette anchors).
   - Pine-only override: `branch.children: {0:14, 1:6, 2:3}` (Pine's EZ-Tree default ships 82 children at level 0; we're still only at ~17% of that, but enough to read as full conifer rather than skeletal stick).
   - Leaf-alpha texture in [`tools/bake-trees/bake.html`](tools/bake-trees/bake.html): 256² → **384²** for crisp mid-distance leaves.
   - Final tree GLB sizes after Draco/Meshopt: tree1 (Aspen) 464 KB · tree2 (Oak) 1.4 MB · pine 265 KB → **2.2 MB total** (was 899 KB).
2. **Floating trees fixed at load-time.** [`js/TerrainBuilder.js`](js/TerrainBuilder.js) tree-load loop now scans for `child.name === 'trunk'` (the bake script names the trunk mesh) and uses ONLY the trunk's `bbox.min.y` as `modelBaseYOffset`. Drooping leaves still contribute to `modelBboxMaxY` for the wind shader's height normalization but no longer drive the ground reference. Console log per tree now reports `(trunk y_min=…, bbox y=[…,…])` so future floating-tree complaints can be triaged from the probe log directly.
3. **Rocks replaced** with procedural in-repo bake. Quaternius MegaKit out, deterministic three.js+simplex-noise rocks in. New tooling at [`tools/bake-rocks.mjs`](tools/bake-rocks.mjs) + [`tools/bake-rocks/bake.html`](tools/bake-rocks/bake.html); same Playwright + static-server pattern as `bake-trees` for code-review symmetry. Three seeded recipes: rock1 (small pebble, flatter), rock2 (cobble, canonical), rock3 (boulder, chunkier high-frequency noise). Common pipeline: `IcosahedronGeometry(0.10, 2)` (180 tris each) → 2-octave 3D simplex displacement (low-freq lumps + high-freq facet break-up) → non-uniform xyz scale for silhouette variety → `toNonIndexed()` + `computeVertexNormals()` for clean flat-shaded faces → per-vertex AO (Y-gradient `0.55→1.00` from bottom to top) baked into vertex colors → `MeshStandardMaterial` with `flatShading: true`. ~11 KB per rock after Draco compress (**~33 KB total**, was 137 KB). Loader code in `TerrainBuilder.createRocks()` is unchanged; same paths, same `_patchRockMaterial` rim-light, same `ROCK_NATIVE_HEIGHT = 0.2m` normalization (which the bake now matches as a near-identity). New `npm run bake-rocks` script alongside `bake-trees`.

Verification: empirical screenshots via the existing probe harness ([`tools/probe.mjs`](tools/probe.mjs)) — `[OK] Loaded tree model: tree1 (trunk y_min=0.000, bbox y=[0.00, 1.00])` × 3 confirm trunk-bbox capture working; `[BUILD] Created … rock{1,2,3} instances` × 3 confirm rocks loaded with correct scaling. Visual diff at 80–200m range: trees read as full canopy silhouettes (vs prior moderately-leaved skeletons), trunks visibly meet terrain (vs floating). The end-to-end `playtest-screenshots` cinema-API harness has a fence-load-related slowness that bumped to 180s timeout; also fixed an old `setCameraPose(s.camera)` → `setCameraPose(s.camera.pos, s.camera.target)` bug in the harness call site.

### Carry-forward work (Phase 5 hero cards, Matt-gated for direction)

The visual followup is shipped; Phase 5 is now the only thing between us and v1.1.0. Steps:

1. **Visual review on the live deploy.** Cloudflare Pages auto-deploys on push to main; eyeball the prod URL. If anything still reads "off," the per-tree trunk_y_min log line in the browser console will help triage floating-tree zones, and the rock recipe in [`tools/bake-rocks.mjs`](tools/bake-rocks.mjs) is parameterized for easy iteration on amplitude / scale / color tints.
