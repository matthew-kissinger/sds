...
import { bloom as bloomStrict } from 'three/addons/tsl/display/BloomNode.js';
...
// The one non-`three/tsl` export here. BloomNode is an addon, but its published
// types are the same strict node types that collapse to `never` under chaining,
// so it belongs at the same cast boundary rather than getting a second one in
// the post-processing component.
export const bloom: (...a: unknown[]) => TSLNode = bloomStrict as never;
