// SPDX-License-Identifier: AGPL-3.0-or-later
// Lifted from sds/shared/Vector2D.js; same license and copyright holder.
// Copyright (c) 2026 Matthew Kissinger
/**
 * Simple 2D Vector class for XZ plane calculations
 * Pure function implementation with no external dependencies
 */

/** Anything with x/z components; lets callers pass plain records or Vector2D. */
type Vec2Like = { x: number; z: number };

export class Vector2D {
    x: number;
    z: number;

    constructor(x: number = 0, z: number = 0) {
        this.x = x;
        this.z = z;
    }

    // Set the x and z components
    set(x: number, z: number): this {
        this.x = x;
        this.z = z;
        return this;
    }

    // Add another vector to this one
    add(vector: Vec2Like): this {
        this.x += vector.x;
        this.z += vector.z;
        return this;
    }

    // Subtract another vector from this one
    subtract(vector: Vec2Like): this {
        this.x -= vector.x;
        this.z -= vector.z;
        return this;
    }

    // Multiply by a scalar
    multiply(scalar: number): this {
        this.x *= scalar;
        this.z *= scalar;
        return this;
    }

    // Divide by a scalar
    divide(scalar: number): this {
        if (scalar !== 0) {
            this.x /= scalar;
            this.z /= scalar;
        }
        return this;
    }

    // Get the magnitude (length) of the vector
    magnitude(): number {
        return Math.sqrt(this.x * this.x + this.z * this.z);
    }

    // Normalize the vector (make it unit length)
    normalize(): this {
        const len = this.magnitude();
        if (len > 0.00001) { // Use a small epsilon to prevent division by zero/tiny number
            this.x /= len;
            this.z /= len;
        } else {
            this.x = 0;
            this.z = 0;
        }
        return this;
    }

    // Limit the magnitude of the vector
    limit(max: number): this {
        const len = this.magnitude();
        if (len > max && len > 0.00001) { // Only normalize if magnitude is significant and over max
            this.normalize();
            this.multiply(max);
        }
        return this;
    }

    // Get distance to another vector
    distanceTo(vector: Vec2Like): number {
        const dx = this.x - vector.x;
        const dz = this.z - vector.z;
        return Math.sqrt(dx * dx + dz * dz);
    }

    // Get the angle of the vector in radians
    // atan2: forbidden in per-tick sim paths; presentation/spawn use only.
    angle(): number {
        return Math.atan2(this.z, this.x);
    }

    // Set the magnitude of the vector
    setMagnitude(magnitude: number): this {
        this.normalize();
        this.multiply(magnitude);
        return this;
    }

    // Create a copy of this vector
    clone(): Vector2D {
        return new Vector2D(this.x, this.z);
    }

    // Static method to create a vector from an angle
    // cos/sin: forbidden in per-tick sim paths; spawn-time/one-shot (seeded, off
    // the tick path) and presentation use only.
    static fromAngle(angle: number): Vector2D {
        return new Vector2D(Math.cos(angle), Math.sin(angle));
    }
}
