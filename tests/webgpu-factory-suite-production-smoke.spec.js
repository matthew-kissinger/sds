...
import { createWebGpuNodeMaterialFactoryGlobals, createWebGpuNodeMaterialFactorySuite } from '../js/webgpuNodeMaterialFactorySuite.js';
...
function createSearch() {
  ...
  ALL_WEBGPU_FLAGS.forEach((flag) => params.set(flag, '1'));
...
describe('webgpu factory suite production smoke', () => {
  it('routes production constructors through suite-backed globals only under explicit flags', () => {
    ...
    try {
      ...
      expect(created.sky.material.name).toBe('webgpu-node-sky-dome');
      expect(created.cloud.material.name).toBe('webgpu-node-cloud-layer');
      expect(created.sun.material.name).toBe('webgpu-node-sun-billboard');
      expect(created.portal.ringMaterial.name).toBe('webgpu-node-portal-ring');
      expect(created.portal.pad.material.name).toBe('webgpu-node-portal-pad');
      expect(created.portal.particles.material.name).toBe('webgpu-node-portal-particles');
      expect(created.zap.effects[0].bolt.material.name).toBe('webgpu-node-corral-zap-bolt');
      expect(created.zap.effects[0].particles.material.name).toBe('webgpu-node-corral-zap-particles');
      expect(created.grassBladeMaterial.name).toBe('webgpu-node-grass-blade');
      expect(created.meadowMaterial.name).toBe('webgpu-node-meadow-quad');
      expect(created.terrainMesh.material.name).toBe('webgpu-node-terrain-heightfield');
      expect(created.water.material.name).toBe('webgpu-node-anime-water');
      expect(created.sheep.material.name).toBe('webgpu-node-sheep-wool');
      expect(created.impostorMaterial.name).toBe('webgpu-node-kiln-impostor');
      expect(created.cloud.webgpuMaterialSummary).toMatchObject({ applied: true });
      expect(created.sun.webgpuMaterialSummary).toMatchObject({ applied: true });
      expect(created.portal.webgpuRingMaterialSummary).toMatchObject({ applied: true });
      expect(created.portal.webgpuPadMaterialSummary).toMatchObject({ applied: true });
      expect(created.portal.webgpuParticleMaterialSummary).toMatchObject({ applied: true });
      expect(created.zap.effects[0].webgpuBoltMaterialSummary).toMatchObject({ applied: true });
      expect(created.zap.effects[0].webgpuParticleMaterialSummary).toMatchObject({ applied: true });
      expect(created.grass.webgpuGrassBladeMaterialSummary).toMatchObject({ applied: true });
      expect(created.grass.webgpuMeadowQuadMaterialSummary).toMatchObject({ applied: true });
      expect(created.terrain.webgpuTerrainMaterialSummary).toMatchObject({ applied: true });
      expect(created.water.webgpuWaterMaterialSummary).toMatchObject({ applied: true });
      expect(created.sheep.webgpuSheepMaterialSummary).toMatchObject({ applied: true });
      expect(created.impostorMaterial.userData.webgpuImpostorMaterialSummary).toMatchObject({ applied: true });
    } finally {
      created.sky?.dispose?.();
