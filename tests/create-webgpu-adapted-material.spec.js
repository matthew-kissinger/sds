...
// [P3-WEBGPU] Shared webgpu adapter helper: factory wiring parity plus
// the new degradation surfacing (console.warn once per material name +
// webgpu_material_degraded telemetry event).
...
import {
    ...
    resetWebGpuMaterialDegradationReportsForTests,
} from '../js/world/createWebGpuAdaptedMaterial.js';

const SEARCH_ON = '?renderer=webgpu&webgpuSpec=1';

function makeAdapter(overrides = {}) {
    return createWebGpuMaterialAdapter({
        flagParam: 'webgpuSpec',
        factoriesGlobal: '__sdsWebGpuSpecMaterialFactories',
        summaryGlobal: null,
        ...overrides,
    });
}

describe('createWebGpuAdaptedMaterial', () => {
    let warnSpy;
    ...
    it('gates on the explicit renderer flag', () => {
        ...
        expect(adapter.shouldApply('?renderer=webgpu&webgpuSpec=1')).toBe(true);
        expect(adapter.shouldApply('?renderer=webgpu')).toBe(false);
        expect(adapter.shouldApply('?renderer=webgl&webgpuSpec=1')).toBe(false);
    ...
    it('returns the factory material with controls when the factory is present', () => {
        ...
        const material = { name: 'webgpu-spec', userData: { specControls: controls } };
        const contexts = [];
    ...
    it('warns once and emits telemetry when the factory is missing', () => {
        ...
        expect(emitEvent).toHaveBeenCalledWith('webgpu_material_degraded', {
            kind: 'spec-kind',
    ...
    it('never reports flag-disabled through the degradation reporter', () => {
        reportWebGpuMaterialDegradation('spec-kind', 'flag-disabled');
        expect(warnSpy).not.toHaveBeenCalled();
        expect(emitEvent).not.toHaveBeenCalled();
    });
});
