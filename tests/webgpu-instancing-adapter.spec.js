...
} from '../js/world/webgpuNativeInstancingAdapter.js';
import { placeEnvironmentDetails } from '../js/world/RockPlacement.js';
import { placeTrees, resolveWebGpuNativeTreeImpostorRoute } from '../js/world/TreePlacement.js';
import {
  createWebGpuTreeImpostorGeometry,
  installWebGpuTreeHybridRuntime,
  installWebGpuTreeImpostorRuntime,
  syncWebGpuTreeHybridVisibility,
  syncWebGpuTreeImpostorMesh,
...
function treeRoot() {
  ...
  const trunk = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ name: 'webgpu-node-branches' })
  );
  trunk.name = 'trunk';
  ...
  const leaves = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ name: 'webgpu-node-leaves' })
  );
  leaves.name = 'leaves';
...
function rockRoot() {
  ...
  const rock = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ name: 'webgpu-node-rock-rim' })
  );
  rock.name = 'rock';
  rock.position.y = 0.3;
  root.add(rock);
...
describe('webgpu native tree instancing adapter', () => {
  it('promotes the explicit production tree impostor route to octahedral v2 with a latlon rollback', () => {
    expect(resolveWebGpuNativeTreeImpostorRoute('?renderer=webgpu&webgpuNativeTreeImpostors=1')).toMatchObject({
      active: true,
      useOctahedral: true,
    ...
    expect(resolveWebGpuNativeTreeImpostorRoute('?renderer=webgpu&webgpuNativeTreeImpostors=latlon')).toMatchObject({
      active: true,
    ...
    expect(resolveWebGpuNativeTreeImpostorRoute('?renderer=webgpu')).toMatchObject({
      active: false,
      sidecarLayout: null,
      sidecarVersion: null,
    });
  ...
  it('builds native Three instanced groups without depending on InstancedMesh2', () => {
    ...
    const result = createWebGpuNativeTreeInstancingPreview({
      scene,
  ...
  it('builds native Three instanced rock groups from diagnostic placement transforms', () => {
    ...
    const result = createWebGpuNativeRockInstancingPreview({
      scene,
  ...
  it('uses native Three instancing for production scene-body tree placement under the guarded flag', async () => {
    ...
    expect(builder.webgpuNativeTreeInstancingSummary).toMatchObject({
      applied: true,
      ok: true,
      source: 'THREE.InstancedMesh',
  ...
  it('syncs production tree impostor tiles and world-up billboard matrices per instance', () => {
    ...
    const runtime = createWebGpuTreeImpostorGeometry(new THREE.PlaneGeometry(1, 1), instances, sidecar);
    const mesh = new THREE.InstancedMesh(runtime.geometry, new THREE.MeshBasicMaterial(), instances.length);
    installWebGpuTreeImpostorRuntime(mesh, {
      ...runtime,
      sidecar,
      treeType: 'tree1',
      chunkKey: '0:0',
    ...
    expect(syncWebGpuTreeImpostorMesh(mesh, camera)).toBe(true);
    const firstOffset = [
    ...
    expect(mesh.userData.webgpuNativeTreeImpostor).toMatchObject({
  ...
  it('keeps near tree chunks geometric and switches far chunks to impostors', () => {
    ...
    installWebGpuTreeHybridRuntime(nearMesh, {
      role: 'near-lod0',
      chunkCenter,
      nearDistance: 5,
      switchDistance: 10,
    });
    installWebGpuTreeHybridRuntime(midMesh, {
      role: 'mid-lod1',
      chunkCenter,
      nearDistance: 5,
      switchDistance: 10,
    });
    installWebGpuTreeHybridRuntime(farMesh, {
      role: 'far-impostor',
      chunkCenter,
      nearDistance: 5,
      switchDistance: 10,
    ...
    expect(syncWebGpuTreeHybridVisibility(nearMesh, camera)).toBe(true);
    expect(syncWebGpuTreeHybridVisibility(midMesh, camera)).toBe(true);
    expect(syncWebGpuTreeHybridVisibility(farMesh, camera)).toBe(true);
    ...
    syncWebGpuTreeHybridVisibility(nearMesh, camera);
    syncWebGpuTreeHybridVisibility(midMesh, camera);
    syncWebGpuTreeHybridVisibility(farMesh, camera);
  ...
  it('uses native Three instancing for production scene-body rock placement under the guarded flag', async () => {
    ...
    expect(builder.webgpuNativeRockInstancingSummary).toMatchObject({
      applied: true,
      ok: true,
      source: 'THREE.InstancedMesh',
      route: 'webgpu-production-scene-body',
    ...
    expect(meshes.some((mesh) => mesh.isInstancedMesh2 === true)).toBe(false);
    expect(scene.children.some((child) => child.isInstancedMesh2 === true)).toBe(false);
    expect(builder.webgpuRockPlacementPlan.totalRocks).toBe(builder.webgpuNativeRockInstancingSummary.rockInstances);
  });
  ...
  it('accepts empty native rock placement when an island scene filters every rock candidate', async () => {
    ...
    expect(builder.webgpuRockPlacementPlan.totalRocks).toBe(0);
    expect(builder.webgpuNativeRockInstancingSummary).toMatchObject({
      applied: true,
      ok: true,
      rockInstances: 0,
