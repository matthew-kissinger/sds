...
test.describe('Newsheepdogland foliage streaming', () => {
  test('streams the island foliage after Play without quality demotion', async ({ page, context, browserName }) => {
    ...
    // Desktop e2e runs at the high tier: streamed grass landed too.
    expect(result.diag.grass?.built).toBe(true);
    // No quality demotion attributable to the streaming window. missWindows
    // is deliberately NOT asserted: it is a transient in-flight counter and a
    // single over-budget window in headless Chromium is environment noise,
    // not a demotion (the step-down requires 2 consecutive misses).
