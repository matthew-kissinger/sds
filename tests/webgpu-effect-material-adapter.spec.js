...
import {
  ...
  shouldApplyWebGpuEffects,
} from '../js/effects/webgpuEffectMaterialAdapter.js';
import { createWebGpuEffectNodeMaterialFactories } from '../js/effects/webgpuEffectNodeMaterialFactories.js';
...
describe('webgpu effect material adapter', () => {
  it('requires the explicit WebGPU effects flag', () => {
    expect(shouldApplyWebGpuEffects('?renderer=webgpu&webgpuEffects=1')).toBe(true);
    expect(shouldApplyWebGpuEffects('?renderer=webgpu&diagnostic=1')).toBe(false);
    expect(shouldApplyWebGpuEffects('?renderer=webgl&webgpuEffects=1')).toBe(false);
    expect(shouldApplyWebGpuEffects('')).toBe(false);
  });

  it('keeps the sun billboard default material unless factories are explicitly supplied', () => {
    const disabled = createWebGpuEffectMaterial('sun-billboard', 'createSunBillboardMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-sun'),
      search: '?renderer=webgpu',
      factories: {
        createSunBillboardMaterial: () => defaultMaterial('webgpu-sun'),
      },
    });
    ...
    const missingFactory = createWebGpuEffectMaterial('sun-billboard', 'createSunBillboardMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-sun'),
  ...
  it('uses explicit sun and portal factories on the WebGPU effect flag', () => {
    ...
    const factories = {
      createSunBillboardMaterial: () => ({
        material: { name: 'webgpu-sun' },
        controls: { update: (state) => sunUpdates.push(state) },
      }),
      createPortalRingMaterial: () => ({
        material: { name: 'webgpu-portal' },
        controls: { update: (state) => portalUpdates.push(state) },
      }),
    };

    const sun = createWebGpuEffectMaterial('sun-billboard', 'createSunBillboardMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-sun'),
      search: '?renderer=webgpu&webgpuEffects=1',
      factories,
    });
    const portal = createWebGpuEffectMaterial('portal-ring', 'createPortalRingMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-portal'),
      search: '?renderer=webgpu&webgpuEffects=1',
      factories,
    });

    expect(sun.material.name).toBe('webgpu-sun');
    ...
    expect(portal.material.name).toBe('webgpu-portal');
  ...
  it('can route production effects through reusable WebGPU node material candidates', () => {
    const factories = createWebGpuEffectNodeMaterialFactories(
    ...
    const pad = createWebGpuEffectMaterial('portal-pad', 'createPortalPadMaterial', {
    ...
    const portalParticles = createWebGpuEffectMaterial('portal-particles', 'createPortalParticleMaterial', {
    ...
    const zapBolt = createWebGpuEffectMaterial('corral-zap-bolt', 'createCorralZapBoltMaterial', {
    ...
    const zapParticles = createWebGpuEffectMaterial('corral-zap-particles', 'createCorralZapParticleMaterial', {
    ...
    try {
      expect(sun.material.name).toBe('webgpu-node-sun-billboard');
      ...
      expect(sun.material.userData.webgpuSunBillboardOwnership).toMatchObject({
        owns: 'disc-body-only',
      ...
      expect(portal.material.name).toBe('webgpu-node-portal-ring');
      ...
      expect(pad.material.name).toBe('webgpu-node-portal-pad');
      ...
      expect(portalParticles.material.name).toBe('webgpu-node-portal-particles');
      ...
      expect(zapBolt.material.name).toBe('webgpu-node-corral-zap-bolt');
      ...
      expect(zapParticles.material.name).toBe('webgpu-node-corral-zap-particles');
  ...
  it('passes effect material context into explicit factories', () => {
    const contexts = [];
    const result = createWebGpuEffectMaterial('portal-pad', 'createPortalPadMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-pad'),
      search: '?renderer=webgpu&webgpuEffects=1',
      factories: {
        createPortalPadMaterial: (context) => {
          contexts.push(context);
          return defaultMaterial('webgpu-pad');
        },
      },
      context: {
        opacity: 0.18,
    ...
    expect(result.material.name).toBe('webgpu-pad');
  ...
  it('routes SunBillboard material creation through the shared adapter', () => {
    ...
    const updates = [];
    const webgpuMaterial = new THREE.MeshBasicMaterial({ name: 'webgpu-sun' });

    const sun = new SunBillboard(scene, {
      search: '?renderer=webgpu&webgpuEffects=1',
      webgpuEffectFactories: {
        createSunBillboardMaterial: () => ({
    ...
    expect(sun.material).toBe(webgpuMaterial);
    expect(sun.webgpuMaterialSummary).toMatchObject({
      kind: 'sun-billboard',
      applied: true,
      reason: null,
      hasControls: true,
  ...
  it('routes PortalEffect pad and particle materials through the shared adapter', () => {
    ...
    const padMaterial = new THREE.MeshBasicMaterial({ name: 'webgpu-portal-pad' });
    const particleMaterial = new THREE.PointsMaterial({ name: 'webgpu-portal-particles' });
    ...
    try {
      ...
      expect(portal.webgpuPadMaterialSummary).toMatchObject({
        kind: 'portal-pad',
        applied: true,
      });
      expect(portal.webgpuParticleMaterialSummary).toMatchObject({
  ...
  it('routes corral zap bolt and particle materials through the shared adapter', () => {
    ...
    const boltUpdates = [];
    const particleUpdates = [];
    const boltMaterial = new THREE.LineBasicMaterial({ name: 'webgpu-zap-bolt' });
    const particleMaterial = new THREE.PointsMaterial({ name: 'webgpu-zap-particles' });
