...
import {
  applyWebGpuTreeRockMaterials,
  maybeApplyWebGpuTreeRockMaterials,
  shouldApplyWebGpuMaterials,
} from '../js/world/webgpuMaterialAdapter.js';
import { TerrainBuilder } from '../js/TerrainBuilder.js';
import { createWebGpuTreeRockNodeMaterialFactories } from '../js/world/webgpuTreeRockNodeMaterialFactories.js';
...
const factories = {
  createTreeBranchMaterial: ({ previous }) => ({ name: 'webgpu-branches', previousName: previous.name }),
  createTreeLeafMaterial: ({ previous }) => ({ name: 'webgpu-leaves', previousName: previous.name }),
  createRockMaterial: ({ previous }) => ({ name: 'webgpu-rock', previousName: previous.name || '(runtime-default)' }),
};

describe('webgpu production material adapter', () => {
  it('requires the explicit WebGPU material flag', () => {
    expect(shouldApplyWebGpuMaterials('?renderer=webgpu&webgpuMaterials=1')).toBe(true);
    expect(shouldApplyWebGpuMaterials('?renderer=webgpu&diagnostic=1')).toBe(false);
    expect(shouldApplyWebGpuMaterials('?renderer=webgl&webgpuMaterials=1')).toBe(false);
    expect(shouldApplyWebGpuMaterials('')).toBe(false);
  });
  ...
  it('replaces tree LOD0/LOD1 by material name and rocks by traversal', () => {
    ...
    const summary = applyWebGpuTreeRockMaterials({
    ...
    expect(treeLod0.children.map((child) => child.material.name)).toEqual(['webgpu-branches', 'webgpu-leaves']);
    expect(treeLod1.children.map((child) => child.material.name)).toEqual(['webgpu-branches', 'webgpu-leaves']);
    expect(rock.children[0].material.name).toBe('webgpu-rock');
  });
  ...
  it('leaves production materials untouched unless flag and factories are both present', () => {
    ...
    expect(maybeApplyWebGpuTreeRockMaterials(builder, {
    ...
    expect(maybeApplyWebGpuTreeRockMaterials(builder, {
      search: '?renderer=webgpu&webgpuMaterials=1',
  ...
  it('applies through the TerrainBuilder-facing adapter when explicitly enabled', () => {
    ...
    const summary = maybeApplyWebGpuTreeRockMaterials(builder, {
      search: '?renderer=webgpu&webgpuMaterials=1',
      factories,
    });
    ...
    expect(summary.ok).toBe(true);
    expect(builder.models.trees.tree1.children[0].material.name).toBe('webgpu-branches');
    expect(builder.models.rocks.rock1.children[0].material.name).toBe('webgpu-rock');
  });
  ...
  it('can route tree and rock replacements through reusable WebGPU node material candidates', () => {
    ...
    const nodeFactories = createWebGpuTreeRockNodeMaterialFactories(
    ...
    try {
      expect(summary.ok).toBe(true);
      expect(branches.name).toBe('webgpu-node-branches');
      ...
      expect(branches.userData.webgpuUsesSourceColor).toBe(true);
      expect(branches.userData.webgpuUsesDistanceFog).toBe(true);
      expect(branches.userData.webgpuTreeWindNodeUniforms).toBeTruthy();
      expect(branches.userData.webgpuTreeNodeMaterialControls?.setWind).toBeInstanceOf(Function);
      ...
      expect(leaves.name).toBe('webgpu-node-leaves');
      ...
      expect(leaves.userData.webgpuUsesSourceMap).toBe(true);
      expect(leaves.userData.webgpuUsesSourceTint).toBe(true);
      expect(leaves.userData.webgpuUsesDistanceFog).toBe(true);
      expect(leaves.userData.webgpuSourceMapScale).toBe(0.58);
      expect(leaves.userData.webgpuTreeWindNodeUniforms).toBeTruthy();
      expect(leaves.userData.webgpuTreeLeafOccluderNodeUniforms).toBeTruthy();
      expect(leaves.userData.webgpuTreeNodeMaterialControls?.setOccluder).toBeInstanceOf(Function);
      expect(rockRim.name).toBe('webgpu-node-rock-rim');
  ...
  it('wires TerrainBuilder model caches through the fail-closed material seam', async () => {
    const builder = new TerrainBuilder(new THREE.Scene(), false, null, {
      search: '?renderer=webgpu&webgpuMaterials=1',
      webgpuMaterialFactories: factories,
    });
    builder.models = {
      trees: { tree1: root(mesh('branches'), mesh('leaves')) },
      treesLod1: { tree1: root(mesh('branches'), mesh('leaves')) },
    ...
    const summary = await builder._applyWebGpuTreeRockMaterials();

    expect(summary).toBe(builder.webgpuTreeRockMaterialSummary);
    expect(summary.applied).toBe(true);
    expect(summary.treeReplacedMaterials).toBe(4);
    expect(summary.rockReplacedMaterials).toBe(1);
