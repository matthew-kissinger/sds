...
import { createWebGpuGrassNodeMaterialFactories } from '../js/world/webgpuGrassNodeMaterialFactories.js';
import {
    createWebGpuGrassMaterial,
    shouldApplyWebGpuGrass,
} from '../js/world/webgpuGrassMaterialAdapter.js';
import { setWebGpuModules } from '../js/world/webgpuModules.js';
...
describe('webgpu grass material adapter', () => {
    afterEach(() => {
        setWebGpuModules(null);
    });

    it('requires the explicit WebGPU grass flag', () => {
        expect(shouldApplyWebGpuGrass('?renderer=webgpu&webgpuGrass=1')).toBe(true);
        expect(shouldApplyWebGpuGrass('?renderer=webgpu&diagnostic=1')).toBe(false);
        expect(shouldApplyWebGpuGrass('?renderer=webgl&webgpuGrass=1')).toBe(false);
        expect(shouldApplyWebGpuGrass('')).toBe(false);
    });
    ...
    it('keeps the coastline compute-cull path eligible on mobile WebGPU', () => {
        ...
        grass.webgpuGrassBladeMaterialSummary = { applied: true };

        setWebGpuModules({ TSL: {} });
    ...
    it('leaves the default meadow quad material untouched without flag and factories', () => {
        ...
        try {
            ...
            expect(grass.webgpuMeadowQuadMaterialSummary).toMatchObject({
                kind: 'meadow-quad',
                applied: false,
    ...
    it('can explicitly opt into the SDS hybrid sparse grass profile', () => {
        ...
        expect(grass.grassProfileSummary).toMatchObject({
            id: 'sds-hybrid-v1',
            clumpDensityScale: 0.68,
            streamedClumpDensityScale: 0.58,
            bladesPerClump: 5,
            sheepFootprint: { halfLen: 0.7, halfWid: 0.48, falloff: 0.82 },
            groundContact: { enabled: true, opacity: 0.16 },
        });
        expect(grass.config.clumpsPerChunk).toBe(1224);
        expect(grass.config.bladesPerClump).toBe(5);
        expect(grass.config.bladeHeight).toBeCloseTo(0.9);
        expect(grass.config.bladeWidth).toBeCloseTo(0.145);
        expect(grass.config.sheepInteractionRadius).toBeCloseTo(1.05);
        expect(grass.config.sheepInteractionStrength).toBeCloseTo(0.58);
        expect(grass.getStats().grassProfile.id).toBe('sds-hybrid-v1');
    });

    it('creates an opt-in hybrid ground contact mesh for the SDS sparse profile', () => {
        const scene = new THREE.Scene();
        ...
        const mesh = grass.createGroundContactMesh();
        try {
            ...
            grass.updateInteractors([
                { position: { x: 1, y: 0, z: 2 }, type: 'dog', currentRotation: Math.PI / 4 },
                { position: { x: 3, y: 0, z: 4 }, type: 'sheep', facingDirection: Math.PI },
            ]);

            expect(mesh.count).toBe(2);
            expect(mesh.visible).toBe(true);
            expect(grass.getStats().groundContact).toMatchObject({
                enabled: true,
                drawCalls: 1,
                instances: 2,
            ...
            expect(grass.setGroundContactEnabled(false)).toBe(false);
            expect(mesh.count).toBe(0);
    ...
    it('routes meadow quad material creation through an explicit WebGPU factory', () => {
        ...
        const grass = new GrassSystem(scene, false, null, null, null, {
            ...
            webgpuGrassFactories: {
                createMeadowQuadMaterial: (context) => {
                    contexts.push(context);
                    return createMaterial('webgpu-meadow-quad');
                },
            },
        });
        ...
        const material = grass.createMeadowQuadMaterial();
        try {
            expect(material.name).toBe('webgpu-meadow-quad');
            expect(grass.webgpuMeadowQuadMaterialSummary).toMatchObject({
                kind: 'meadow-quad',
                applied: true,
            });
    ...
    it('can route the meadow quad through the reusable WebGPU node material candidate', () => {
        const contexts = [];
        const nodeFactories = createWebGpuGrassNodeMaterialFactories(
        ...
        const material = grass.createMeadowQuadMaterial();
        try {
            expect(material.name).toBe('webgpu-node-meadow-quad');
    ...
    it('routes grass blade material creation through an explicit WebGPU factory', () => {
        ...
        const grass = new GrassSystem(scene, false, null, null, null, {
            ...
            webgpuGrassFactories: {
                createGrassBladeMaterial: (context) => {
                    contexts.push(context);
                    return createMaterial('webgpu-grass-blade');
                },
            },
        });
        ...
        const material = grass.createGrassMaterial();
        try {
            expect(material.name).toBe('webgpu-grass-blade');
            expect(grass.webgpuGrassBladeMaterialSummary).toMatchObject({
                kind: 'grass-blade',
                applied: true,
            });
    ...
    it('passes the SDS hybrid profile to WebGPU grass blade factories', () => {
        const contexts = [];
        const grass = new GrassSystem(new THREE.Scene(), false, null, null, null, {
            search: '?renderer=webgpu&webgpuGrass=1&grassProfile=sds-hybrid-v1',
            webgpuGrassFactories: {
                createGrassBladeMaterial: (context) => {
                    contexts.push(context);
                    return createMaterial('webgpu-grass-blade-profile');
                },
            },
        });
        ...
        const material = grass.createGrassMaterial();
        try {
            expect(material.name).toBe('webgpu-grass-blade-profile');
            expect(contexts).toHaveLength(1);
            expect(contexts[0].grassProfile.id).toBe('sds-hybrid-v1');
            expect(contexts[0].geometry.bladeHeight).toBeCloseTo(0.9);
            expect(contexts[0].interaction.visualScale).toBeCloseTo(7.1);
            expect(contexts[0].interaction.laydownStrength).toBeCloseTo(1.05);
            expect(contexts[0].interaction.maxDisplacement).toBeCloseTo(1.35);
            expect(contexts[0].interaction.shadowStrength).toBeCloseTo(0.18);
            expect(contexts[0].interaction.sheepHalfLen).toBeCloseTo(0.7);
            expect(contexts[0].interaction.sheepHalfWid).toBeCloseTo(0.48);
            expect(contexts[0].tipDampen).toBeCloseTo(0.44);
            expect(contexts[0].rimStrength).toBeCloseTo(0.14);
        } finally {
            material.dispose();
        }
    });
    ...
    it('can route grass blades through the reusable WebGPU node material candidate', () => {
        ...
        try {
            ...
            expect(material.userData.webgpuGrassBladeInteractors).toMatchObject({
    ...
    it('adds per-clump world offsets to WebGPU grass chunks for contact bending', () => {
        ...
        grass.grassMaterial = createMaterial('webgpu-grass-blade');
        grass.webgpuGrassBladeMaterialSummary = { applied: true };
    ...
    it('copies dog plus sheep interactors into the WebGPU node control packet', () => {
        ...
        const material = nodeFactories.createGrassBladeMaterial({
            tier: 'high',
            interaction: { maxInteractors: 10, maxDisplacement: 1.35 },
        });

        try {
            const controls = material.userData.webgpuGrassBladeMaterialControls;
            ...
            expect(material.userData.webgpuGrassBladeInteractors.maxNodeInteractors).toBe(8);
            expect(material.userData.webgpuGrassBladeInteractors.maxDisplacement).toBe(1.35);
        ...
        const grass = new GrassSystem(scene, false, null, null, null, {
            ...
            webgpuGrassFactories: {
                createGrassBladeMaterial: () => ({
                    material: createMaterial('webgpu-grass-blade-controls'),
                    controls: {
    ...
    it('falls back to default meadow material when a factory is missing or invalid', () => {
        const missing = createWebGpuGrassMaterial('meadow-quad', 'createMeadowQuadMaterial', {
        ...
        const invalid = createWebGpuGrassMaterial('meadow-quad', 'createMeadowQuadMaterial', {
