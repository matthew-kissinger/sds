...
import {
  ...
  shouldApplyWebGpuAtmosphere,
} from '../js/atmosphere/webgpuAtmosphereMaterialAdapter.js';
import {
  createWebGpuCloudLayerMaterialFactories,
  createWebGpuCloudLayerNodeMaterial,
} from '../js/atmosphere/webgpuCloudNodeMaterial.js';
import {
  createWebGpuSkyDomeMaterialFactories,
  createWebGpuSkyFogNodeMaterial,
} from '../js/atmosphere/webgpuSkyNodeMaterial.js';
...
describe('webgpu atmosphere material adapter', () => {
  it('requires the explicit WebGPU atmosphere flag', () => {
    expect(shouldApplyWebGpuAtmosphere('?renderer=webgpu&webgpuAtmosphere=1')).toBe(true);
    expect(shouldApplyWebGpuAtmosphere('?renderer=webgpu&diagnostic=1')).toBe(false);
    expect(shouldApplyWebGpuAtmosphere('?renderer=webgl&webgpuAtmosphere=1')).toBe(false);
    expect(shouldApplyWebGpuAtmosphere('')).toBe(false);
  });

  it('keeps the default sky material unless factories are explicitly supplied', () => {
    const disabled = createWebGpuAtmosphereMaterial('sky-dome', 'createSkyDomeMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-sky'),
      search: '?renderer=webgpu',
      factories: {
        createSkyDomeMaterial: () => defaultMaterial('webgpu-sky'),
      },
    });
    ...
    const missingFactory = createWebGpuAtmosphereMaterial('sky-dome', 'createSkyDomeMaterial', {
      createDefaultMaterial: () => defaultMaterial('default-sky'),
      search: '?renderer=webgpu&webgpuAtmosphere=1',
    });
  ...
  it('allows HosekWilkieSky to receive an explicit WebGPU sky material factory', () => {
        ...
        createWebGpuAtmosphereMaterial('sky-dome', 'createSkyDomeMaterial', {
          ...
          factories: {
            createSkyDomeMaterial: (factoryContext) => {
              ...
              return {
                material: defaultMaterial('webgpu-sky-dome'),
              };
            },
          },
          context,
    ...
    try {
      expect(sky.material.name).toBe('webgpu-sky-dome');
  ...
  it('routes HosekWilkieSky through the shared atmosphere adapter when no override factory is supplied', () => {
    ...
    const sky = new HosekWilkieSky({
      search: '?renderer=webgpu&webgpuAtmosphere=1',
      webgpuAtmosphereFactories: {
        createSkyDomeMaterial: (factoryContext) => {
          contexts.push(factoryContext);
          return defaultMaterial('webgpu-direct-sky');
        },
      },
    });

    try {
      expect(sky.material.name).toBe('webgpu-direct-sky');
      expect(contexts).toHaveLength(1);
      expect(contexts[0].uniforms).toBe(sky.uniforms);
    } finally {
      sky.dispose();
  ...
  it('creates a reusable WebGPU sky node material from the CPU sky fog packet', () => {
    ...
    const material = createWebGpuSkyFogNodeMaterial(WEBGPU, skyFog);

    expect(material.name).toBe('webgpu-node-sky-fog');
    expect(material.isNodeMaterial).toBe(true);
    expect(material.depthWrite).toBe(false);
    expect(material.depthTest).toBe(false);
    expect(material.userData.webgpuSkyMaterialControls?.update).toBeInstanceOf(Function);
    expect(material.userData.webgpuSkyNodeUniforms.sunDirection.value.toArray()).toEqual(
      new THREE.Vector3(...skyFog.sunDirection).normalize().toArray()
    );
    expect(material.userData.webgpuSkyPresetTuning).toMatchObject({
  ...
  it('routes HosekWilkieSky to the extracted WebGPU sky node material factory under the explicit flag', () => {
    ...
    const sky = new HosekWilkieSky({
      search: '?renderer=webgpu&webgpuAtmosphere=1',
      webgpuAtmosphereFactories: createWebGpuSkyDomeMaterialFactories(WEBGPU, skyFog),
    });

    try {
      expect(sky.material.name).toBe('webgpu-node-sky-dome');
      expect(sky.material.isNodeMaterial).toBe(true);
      expect(sky.material.side).toBe(WEBGPU.BackSide);
      expect(sky.materialControls).toBe(sky.material.userData.webgpuSkyMaterialControls);
  ...
  it('lets Atmosphere forward an explicit sky material factory without changing defaults', () => {
      ...
      skyFactory: (context) =>
        createWebGpuAtmosphereMaterial('sky-dome', 'createSkyDomeMaterial', {
          ...
          factories: {
            createSkyDomeMaterial: (factoryContext) => {
              contexts.push(factoryContext);
              return defaultMaterial('webgpu-atmosphere-sky');
            },
          },
          context,
        }),
    ...
    try {
      expect(atmo.sky.material.name).toBe('webgpu-atmosphere-sky');
      expect(contexts).toHaveLength(1);
  ...
  it('routes CloudLayer through the shared atmosphere adapter with update controls', () => {
    ...
    const layer = new CloudLayer({
      ...
      webgpuAtmosphereFactories: {
        createCloudLayerMaterial: (factoryContext) => {
          contexts.push(factoryContext);
          return {
            material: new THREE.MeshBasicMaterial({ name: 'webgpu-cloud-layer' }),
            controls: {
    ...
    try {
      expect(layer.material.name).toBe('webgpu-cloud-layer');
      expect(layer.webgpuMaterialSummary).toMatchObject({
        kind: 'cloud-layer',
        applied: true,
      });
      expect(contexts).toHaveLength(1);
  ...
  it('creates a reusable WebGPU cloud node material', () => {
    const material = createWebGpuCloudLayerNodeMaterial(WEBGPU);

    expect(material.name).toBe('webgpu-node-cloud-layer');
    expect(material.isNodeMaterial).toBe(true);
  ...
  it('routes CloudLayer to the extracted WebGPU cloud node material factory under the explicit flag', () => {
    const layer = new CloudLayer({
      search: '?renderer=webgpu&webgpuAtmosphere=1',
      webgpuAtmosphereFactories: createWebGpuCloudLayerMaterialFactories(WEBGPU),
    });

    try {
      expect(layer.material.name).toBe('webgpu-node-cloud-layer');
      ...
      expect(layer.materialControls).toBe(layer.material.userData.webgpuCloudLayerMaterialControls);
      expect(layer.material.userData.webgpuCloudLayerMaterialSummary).toMatchObject({
        kind: 'cloud-layer',
        applied: true,
      });
  ...
  it('lets Atmosphere forward an explicit cloud material factory without changing defaults', () => {
    ...
    const atmo = new Atmosphere(scene, {
      ...
      attachFog: false,
      cloudFactory: () => ({
        material: new THREE.MeshBasicMaterial({ name: 'webgpu-atmosphere-cloud' }),
        controls: {
    ...
    try {
      expect(atmo.cloudLayer.material.name).toBe('webgpu-atmosphere-cloud');
