...
import {
    createWebGpuSheepMaterial,
    shouldApplyWebGpuSheep,
} from '../js/webgpuSheepMaterialAdapter.js';
...
describe('webgpu sheep material adapter', () => {
    it('requires the explicit WebGPU sheep flag', () => {
        expect(shouldApplyWebGpuSheep('?renderer=webgpu&webgpuSheep=1')).toBe(true);
        expect(shouldApplyWebGpuSheep('?renderer=webgpu&diagnostic=1')).toBe(false);
        expect(shouldApplyWebGpuSheep('?renderer=webgl&webgpuSheep=1')).toBe(false);
        expect(shouldApplyWebGpuSheep('')).toBe(false);
    });
    ...
    it('leaves the default optimized sheep material untouched without flag and factories', () => {
        ...
        try {
            ...
            expect(sheep.webgpuSheepMaterialSummary).toMatchObject({
                kind: 'sheep-wool',
                applied: false,
    ...
    it('routes optimized sheep material creation through an explicit WebGPU factory', () => {
        ...
        const sheep = new OptimizedSheepSystem(scene, 2, null, false, {
            ...
            webgpuSheepFactories: {
                createSheepMaterial: (context) => {
                    contexts.push(context);
                    return createMaterial('webgpu-sheep-wool');
                },
            },
        });

        try {
            expect(sheep.material.name).toBe('webgpu-sheep-wool');
            expect(sheep.instancedMesh.material).toBe(sheep.material);
            expect(sheep.webgpuSheepMaterialSummary).toMatchObject({
                kind: 'sheep-wool',
                applied: true,
    ...
    it('can route optimized sheep through the reusable WebGPU wool node material candidate', () => {
        ...
        try {
            expect(sheep.material.name).toBe('webgpu-node-sheep-wool');
            ...
            expect(sheep.material.userData.webgpuSheepWool).toMatchObject({
            ...
            expect(sheep.material.userData.webgpuSheepAnimation).toMatchObject({
    ...
    it('uses factory controls for sheep material updates', () => {
        ...
        const sheep = new OptimizedSheepSystem(scene, 0, null, false, {
            ...
            webgpuSheepFactories: {
                createSheepMaterial: () => ({
                    material: createMaterial('webgpu-sheep-controls'),
                    controls: {
        ...
        expect(calls[0][1]).toMatchObject({
            deltaTime: 0.25,
            sceneFog: scene.fog,
            material: expect.objectContaining({ name: 'webgpu-sheep-controls' }),
        });
    });
    ...
    it('uses built-in WebGPU sheep controls for scene fog updates', () => {
        ...
        try {
            const controls = sheep.webgpuSheepMaterialControls;
    ...
    it('falls back to the default sheep material when a factory is missing or invalid', () => {
        const missing = createWebGpuSheepMaterial('sheep-wool', 'createSheepMaterial', {
        ...
        const invalid = createWebGpuSheepMaterial('sheep-wool', 'createSheepMaterial', {
