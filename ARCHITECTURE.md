...
- Three.js scene + WebGL renderer + lighting setup. Production renderer setup
  is delegated to `js/rendering/sceneRendererSetup.js`, which keeps the default
  `THREE.WebGLRenderer` path while isolating guarded WebGL capability logging,
  context-loss handlers, shadow/pixel-ratio setup, and tonemapping selection.
  `SceneManager` accepts an explicit renderer factory only for proof runs; the
  normal production call site still uses the default WebGL setup. Konveyor's
  current WebGPU proof uses that factory seam only under
  `?renderer=webgpu&diagnostic=1&konveyorSceneManagerProof=1`, where production
  `SceneManager.whenRendererReady()` owns the injected `WebGPURenderer` init
  step, `SceneManager.render()` routes it through `renderAsync` without
  overlapping frames, and production
  `Atmosphere` sky/cloud/fog, `SunBillboard`,
  `TerrainBuilder.createTerrain()`, `AnimeWater.createAnimeWater()`,
  ...
  slice are routed through the diagnostic-installed
  `window.__sdsKonveyor*MaterialFactories` WebGPU supply inside the injected
  `SceneManager` scene. The
...
- **Far-tree LOD** via 3-quad impostors. Each tree GLB is rendered once to a 512² RenderTarget by an offscreen orthographic camera (transparent background, `MeshBasicMaterial { alphaTest: 0.4, transparent: true }`). Far instances use a 3-plane geometry at 60° apart sharing that texture — any view direction is within 30° of one quad's normal, so the silhouette never goes edge-on. ~99% triangle reduction in the farField + horizon zones. The WebGL / per-chunk path switches at a static **distance-from-origin** (`FAR_LOD_DIST`, ~250m). The consolidated WebGPU path (`js/world/treeComputeCull.js`, with `grassComputeCull.js`) is the exception: it is **camera-relative**, rendering LOD0 within `CONSOLIDATED_FAR_SWITCH_DISTANCE` (200m of the camera) and kiln cross-billboard impostors beyond, selected per instance per frame as pure data-compaction (instance counts change, meshes stay pinned), so the mesh-swap churn the origin-distance rule guards against cannot occur. Newsheepdogland uses the consolidated path.
...
- **Shadow-pass override churn fix** (`js/rendering/shadowOverrideMaterialFix.js`, Cycle 92). r184 renders shadow casters through one shared override `NodeMaterial` per light; `Material.alphaTest`'s setter bumps `version` on every zero-crossing, and NSL mixes alphaTest 0 (trunks, dog, fences) with 0.4 (canopy billboards), so every shadow render object recomputed its pipeline cache key every frame (~221/frame, GC-correlated hitches). `suppressShadowOverrideVersionChurn(material)` installs a non-bumping `alphaTest` accessor on that one instance via `onBeforeShadow`. Result: churn down ~92%, NSL 1%-low 70 to 133-140. The Cycle 96 NSL jitter rail (`npm run perf:jitter:nsl`) guards this floor.
...
- **Shipped biomes** — `field` ("Home Field" - flat fenced starter pasture with a perimeter pen), `rolling-hills` ("Rolling Hills" - 180 m heightfield island with water, sunset atmosphere, and lightning corral), `open-country` ("Open Country" - 380 m island with a multi-stage round-up then portal objective), and `newsheepdogland` ("Newsheepdogland" - boot-shaped survival island with a northern mountain, a homestead pen, a day/night cycle, and wolves after dark; the WebGPU flagship, Experimental pill removed in Cycle 92). All four have `Atmosphere` wired; island scenes use scene-defined boundaries, terrain-aware structure placement, water/shoreline behavior, and obstacle-aware tree/rock placement. Newsheepdogland is a **streamed** scene (impostor-first cold coverage, post-Play LOD0 waves); the other three are all-cold.
...
The shipped content is now four differentiated biomes, not a single field prototype. Home Field teaches the classic pen loop, Rolling Hills proves the island/corral/water atmosphere stack, Open Country proves larger-scale multi-stage objectives, and Newsheepdogland proves the streamed-foliage survival island (day/night, wolves, flock growth). The modules are designed to add scenes and modes by data plus narrow render hooks, not by rebuilding the orchestrator.
