...
const cuts = [
  ...
  { take: 'offline-finish', start: 0, duration: 5, title: 'end', stillAt: 4.983333, crop: '1440:810:240:20' },
];
...
for (const [i, cut] of cuts.entries()) {
  ...
  const args = ['-ss', String(cut.stillAt ?? cut.start), '-i', join(dir, `${cut.take}.mp4`)];
  if (cut.title) args.push('-loop', '1', '-i', join(dir, 'titles', `${cut.title}.png`));
  const video = `[0:v]${cut.stillAt !== undefined ? `trim=end_frame=1,tpad=stop_mode=clone:stop_duration=${cut.duration},` : ''}fps=60,setpts=PTS-STARTPTS,${cut.crop ? `crop=${cut.crop},` : ''}scale=1920:1080:force_original_aspect_ratio=increase:flags=lanczos,crop=1920:1080,setsar=1[v]${cut.title ? `;[v][1:v]overlay=0:0:shortest=1:enable='gte(t,${cut.titleStart ?? 0})'[out]` : ';[v]null[out]'}`;
  args.push('-filter_complex_threads', '2', '-filter_complex', video, '-map', '[out]', '-map', '0:a:0',
    '-t', String(cut.duration), '-af', `apad,afade=t=in:d=0.035,afade=t=out:st=${cut.duration - 0.06}:d=0.06`,
...
for (const track of tracks) {
  ...
  const filter = '[1:a]loudnorm=I=-21:TP=-3:LRA=11,afade=t=in:d=0.35,afade=t=out:st=39:d=3[m];[m][2:a]amix=inputs=2:duration=longest:normalize=0,alimiter=limit=0.84:level=false[a]';
  ffmpeg(['-i', base, '-ss', String(track.start), '-i', music, '-i', join(dir, 'game-audio.wav'), '-filter_complex', filter, '-map', '0:v:0', '-map', '[a]',
    ...
    join(deliver, `sheepdog-sim-v4-${track.id}.mp4`)]);
...
ffmpeg(['-i', base, '-an', '-c:v', 'copy', '-movflags', '+faststart', join(deliver, 'sheepdog-sim-v4-silent.mp4')]);
...
ffmpeg(['-i', teaser, '-ss', '0', '-i', join(dir, 'music', 'bossabossa.mp3'), '-i', join(dir, 'teaser-game-audio.wav'), '-filter_complex',
  '[1:a]loudnorm=I=-21:TP=-3:LRA=11,afade=t=in:d=0.25,afade=t=out:st=11.5:d=3[m];[m][2:a]amix=inputs=2:duration=longest:normalize=0,alimiter=limit=0.84:level=false[a]',
  '-map', '0:v', '-map', '[a]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '256k', '-ar', '48000', '-t', '15', '-movflags', '+faststart', join(deliver, 'sheepdog-sim-v4-teaser-15s.mp4')]);
