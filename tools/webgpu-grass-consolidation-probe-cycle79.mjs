...
// Cycle 79 P1 spike - does GRASS MESH-COUNT CONSOLIDATION clear the nsl cold-load
// budget on WebGPU? Cycle 78 proved each per-chunk THREE.InstancedMesh mints its
// own node pipeline (unique NodeBuffer_<id> uniform-buffer name), so nsl's ~745
// grass chunks -> ~1034 distinct pipelines (76s compile) + a per-mesh resource
// build block. InstancedMesh2 is REFUTED for this (WebGL-only lib). The lever is
// MESH COUNT on the stock-InstancedMesh node path.
//
// This probe coarsens grass chunks (js/GrassSystem.js, flag-gated on
// globalThis.__SDS_GRASS_CHUNK_SCALE = N: chunkSize*N, clumpsPerChunk*N^2, holding
// total density) and measures BOTH:
//   - the collapse mechanism: distinct WGSL + render-pipeline count + InstancedMesh
//     count (Dawn-boundary instrumentation + scene walk)
//   - the hard-stop-1 gate: worst main-thread long-task (TDR proxy), time-to-stable
//     (>=55fps for 2s), build/ready marks, steady fps, errors
//
// SCALE=1 is the 745-chunk baseline. RENDERER=webgl gives the 491ms budget anchor.
// Needs the dev server (SDS_SUPPRESS_BROWSER_OPEN=1 npx vite --port 3000) and, for
// webgpu, the nsl `renderer: 'webgl'` pin temporarily lifted (restore byte-identical).
//
// Usage: RENDERER=webgpu SCALE=4 node tools/webgpu-grass-consolidation-probe-cycle79.mjs
...
const OUT_DIR = resolve(__dirname, '..', 'cycle79-validation');
const PROFILE = resolve(tmpdir(), 'sds-c79-grass');
...
const TREE_SCALE = Number(process.env.TREE_SCALE || '1');
const TAG = `${RENDERER}-s${SCALE}-t${TREE_SCALE}`;
const BASE = `http://localhost:3000/?renderer=${RENDERER}`;
const TIMEOUT = 200_000;

// Runs in the page before any app code. Sets the spike flag + wraps the GPU device
// (pipeline/shader-module counters) + installs the long-task observer.
function installInstrumentation(cfg) {
    const W = window;
    W.__SDS_GRASS_CHUNK_SCALE = cfg.scale > 1 ? cfg.scale : undefined;
    W.__SDS_TREE_CHUNK_SCALE = cfg.treeScale > 1 ? cfg.treeScale : undefined;
    W.__longTasks = [];
...
async function walkScene(page) {
    return page.evaluate(() => {
        ...
        const byMaterial = {};
        let instMeshes = 0, totalCap = 0, totalDraw = 0;
        scene.traverse((o) => {
        ...
        return { instMeshes, totalCap, totalDraw, byMaterial };
    });
}
...
async function run() {
    ...
    const result = { renderer: RENDERER, scale: SCALE, treeScale: TREE_SCALE, startedAt: new Date().toISOString() };
    const ctx = await chromium.launchPersistentContext(PROFILE, { channel: 'chrome', args: GPU_ARGS });
    const page = await ctx.newPage();
    await page.addInitScript(installInstrumentation, { scale: SCALE, treeScale: TREE_SCALE });
    ...
    const collect = (tag, text) => { const cat = categorize(text); if (!cat) return; counts[cat]++; if (samples[cat].length < 5) samples[cat].push(`[${tag}] ${text.slice(0, 180)}`); };
    ...
    try {
        ...
        result.scaleFlag = await page.evaluate(() => window.__SDS_GRASS_CHUNK_SCALE ?? null);
        result.treeScaleFlag = await page.evaluate(() => window.__SDS_TREE_CHUNK_SCALE ?? null);
        result.uniformLimit = await page.evaluate(() => { try { return window.gameInstance?.sceneManager?.renderer?.backend?.device?.limits?.maxUniformBufferBindingSize ?? null; } catch { return null; } });
        ...
        try {
            ...
            await writeFile(resolve(OUT_DIR, `grass-${TAG}.png`), Buffer.from(data, 'base64'));
    ...
    await mkdir(OUT_DIR, { recursive: true });
    await writeFile(resolve(OUT_DIR, `grass-${TAG}.json`), JSON.stringify(result, null, 2));
    console.log(JSON.stringify({
        renderer: RENDERER, scale: SCALE, treeScale: TREE_SCALE, effective: result.effective,
        scaleFlag: result.scaleFlag, treeScaleFlag: result.treeScaleFlag,
        uniformLimit: result.uniformLimit, cold: result.cold, nsl: result.nsl, sceneWalk: result.sceneWalk,
        errorCounts: counts, crashed: result.crashed ?? false, fatal: result.fatal ?? null,
    }, null, 2));
    console.log('Wrote', resolve(OUT_DIR, `grass-${TAG}.json`));
}
run().catch((e) => { console.error('[GRASS-C79] fatal', e); process.exit(1); });
