...
# Frame-stability (jitter) regression rails - RTX 3070 desktop; build + preview first, headed Chrome.
npm run perf:jitter -- --check=1       # field/practice gate (cycle89-validation/jitter-budgets.json)
npm run perf:jitter:nsl -- --check=1   # NSL survival gate; the first battery after a fresh build is cold, run twice and gate on the rerun
```
