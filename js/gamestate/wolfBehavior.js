...
/**
 * Cycle 66 P4: pure wolf-AI helpers, split out of js/gamestate/wolfPack.js so
 * the decision math is testable in Node without importing Three.js (wolfPack
 * pulls js/Wolf.js -> three). No DOM, no Three, no Math.random here - the pack
 * passes a seeded RNG in where randomness is needed.
 *
 * CLIENT-ONLY (the survival cycle is solo; the deterministic `shared/` sim is
 * untouched). These helpers never run in the Worker; they shape the local-only
 * predator layer that sits on top of the shared sheep sim.
 */

/**
 * How many wolves spawn on a given survival night. Escalates per day from a
 * small starter pack to a hard ceiling (Q3 strawman: 2 on night one, +1/day,
 * cap 8). Exposed as opts so Matt's feel pass can retune without code change.
