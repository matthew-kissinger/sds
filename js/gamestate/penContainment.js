 ...
 *
 * A CLIENT-ONLY per-frame correction, deliberately NOT a `shared/` deterministic
 * module - it mirrors the dayLoop / countingMode precedent: solo + client-side,
 * so there is no sim-baseline regen and no multiplayer desync surface. (Co-op
 * promotes pen containment into `shared/` in a later cycle.)
 *
...
export class PenContainment {
    ...
    /** A random settle spot inside the pen, inset from the fence and the gate. */
    _settleSpot() {
        const inset = 4;
        const x = this.minX + inset + Math.random() * Math.max(0.1, (this.maxX - this.minX) - 2 * inset);
        const z = this.minZ + inset + Math.random() * Math.max(0.1, (this.maxZ - this.minZ) - 2 * inset);
        ...
        if (Array.isArray(sheep)) {
            for (let i = 0; i < sheep.length; i++) {
                ...
                if (this._isInside(p.x, p.z)) {
                    ...
                    s._penSettling = true;
                    s.settleTarget = this._settleSpot();
                    if (s.velocity) s.velocity.set(0, 0);
                    penned++;
                    continue;
                }
