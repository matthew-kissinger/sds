...
const FLAG_PARAM = 'konveyorImpostors';
...
function getWindowFactories() {
  ...
  return window.__sdsKonveyorImpostorMaterialFactories ?? null;
}

function exposeSummary(summary) {
  if (typeof window !== 'undefined') {
    window.__sdsKonveyorImpostorMaterialAdapter = summary;
  }
}
...
export function shouldApplyKonveyorImpostors(search = getWindowSearch()) {
  return shouldApplyKonveyorRendererFlag(search, FLAG_PARAM);
}

export function createKonveyorImpostorMaterial(kind, factoryName, {
  createDefaultMaterial,
  search = getWindowSearch(),
  factories = getWindowFactories(),
  context = {},
} = {}) {
  if (!shouldApplyKonveyorImpostors(search)) {
