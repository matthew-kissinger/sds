 ...
 *
 * Cycle 48 P1: converted to JSX .tsx. Behavior-identical; the panel chrome is
 * the shared HudPanel. The pause button keeps its bespoke 32x32 rgba styling
 * (not the pill-radius IconButton primitive) so the look is unchanged.
 * Positioning lives in HudLayout's topLeft slot (Cycle 35 Phase 8).
 */
...
const PauseIcon = ({ size = 16, color = 'currentColor' }: { size?: number; color?: string }) => (
        ...
        <HudPanel className="p-4 min-w-[200px]">
            <div className="flex items-center gap-3">
                <div className="opacity-90">
                    <SheepIcon size={28} color="rgba(255, 255, 255, 0.85)" />
                </div>
                <div>
                    <div className="text-md text-white font-semibold">{sheepCount} / {totalSheep}</div>
                    <div className="text-sm text-blue-300">{percentage}% {t('hud.complete')}</div>
                </div>
                {onPause && (
                    <button
                        type="button"
                    ...
                    >
                        <PauseIcon size={16} color="rgba(255, 255, 255, 0.9)" />
                    </button>
                )}
            </div>
