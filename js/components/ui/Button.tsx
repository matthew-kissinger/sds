 ...
 *
 * Cycle 47 P4: converted from createElement .js to token-driven .tsx. The
 * variant palette now reads design tokens (white text, danger label shades)
 * instead of raw hex literals; the glass rgba surfaces are unchanged. Behavior
 * is identical to the previous Button.js.
 */
...
const SIZE_OVERRIDES: Record<ButtonSize, { padding: string; fontSize: string; radius: string }> = {
...
const VARIANT_THEME: Record<ButtonVariant, VariantTheme> = {
    primary: {
    ...
    danger: {
        ...
        colorHover: color.dangerTextStrong,
...
type ResponsiveInfo = { isLandscapeMobile?: boolean; isCompact?: boolean };

function pickResponsive(responsive: ResponsiveInfo, values: { default: string; compact?: string; landscape?: string }) {
...
function computeButtonStyle({ variant, size, responsive, isHovered, fullWidth, disabled }: ComputeArgs): CSSProperties {
...
export interface BackButtonProps extends Omit<ButtonProps, 'variant' | 'fullWidth'> {
    children?: ReactNode;
}

export function BackButton({ children = 'Back', onClick, ...props }: BackButtonProps) {
    return (
        <Button variant="secondary" fullWidth onClick={onClick} {...props}>
            <span>←</span>
            <span>{children}</span>
        </Button>
    );
