...
export function createWebGpuTreeLeafNodeMaterial({ MeshStandardNodeMaterial, DoubleSide, TSL }, treeLeaf) {
  ...
  material.name = 'webgpu-node-leaves';
  ...
  material.userData.webgpuSourceMapScale = treeLeaf.sourceMapScale ?? 0.58;
  material.userData.webgpuLeafColorScale = colorScale;
  material.userData.webgpuLeafAlphaScale = alphaScale;
  material.userData.webgpuTreeWindNodeUniforms = {
    windDirection,
    windStrength,
  };
  material.userData.webgpuTreeLeafOccluderNodeUniforms = {
    occluderDogVS,
    occluderRadius,
    occluderStrength,
    occluderPeak,
  };
  material.userData.webgpuTreeNodeMaterialControls = createTreeLeafNodeMaterialControls(material);
...
function createTreeLeafNodeMaterialControls(material) {
  ...
  const occluder = material.userData.webgpuTreeLeafOccluderNodeUniforms;
  return {
    wind,
    occluder,
    setWind(state = {}) {
