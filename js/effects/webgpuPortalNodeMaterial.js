...
export function createWebGpuPortalRingNodeMaterial({ MeshBasicNodeMaterial, AdditiveBlending, DoubleSide, TSL }, portal = {}) {
  ...
  material.name = 'webgpu-node-portal-ring';
  ...
  material.userData.webgpuTimeUniform = timeUniform;
  material.userData.webgpuPulseUniform = pulseUniform;
  material.userData.webgpuIntensityUniform = intensityUniform;
  return material;
}

export function createWebGpuPortalPadNodeMaterial({ MeshBasicNodeMaterial, AdditiveBlending, DoubleSide, TSL }, portal = {}) {
  const { uniform, vec3 } = TSL;
  const opacity = uniform(portal.opacity ?? 0.18);

  const material = new MeshBasicNodeMaterial();
  material.name = 'webgpu-node-portal-pad';
  ...
  material.userData.webgpuBaseOpacity = portal.opacity ?? 0.18;
  return material;
}

export function createWebGpuPortalParticleNodeMaterial({ PointsNodeMaterial, AdditiveBlending, TSL }, portal = {}) {
  const { uniform, vec3 } = TSL;
  const opacity = uniform(portal.opacity ?? 0.9);

  const material = new PointsNodeMaterial();
  material.name = 'webgpu-node-portal-particles';
  ...
  material.blending = portal.blending ?? AdditiveBlending;
  material.userData.webgpuOpacityUniform = opacity;
  material.userData.webgpuBaseOpacity = portal.opacity ?? 0.9;
  return material;
}
