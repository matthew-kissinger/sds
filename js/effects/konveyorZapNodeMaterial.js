...
export function createKonveyorCorralZapBoltNodeMaterial({ LineBasicNodeMaterial, TSL }, zap = {}) {
  const { uniform, vec3 } = TSL;
  const opacity = uniform(zap.opacity ?? 0);

  const material = new LineBasicNodeMaterial();
  material.name = 'konveyor-node-corral-zap-bolt';
  ...
  material.userData.konveyorBaseOpacity = zap.opacity ?? 0;
  return material;
}

export function createKonveyorCorralZapParticleNodeMaterial({ PointsNodeMaterial, TSL }, zap = {}) {
  const { uniform, vec3 } = TSL;
  const opacity = uniform(zap.opacity ?? 0);

  const material = new PointsNodeMaterial();
  material.name = 'konveyor-node-corral-zap-particles';
