...
const FLAG_PARAM = 'konveyorAtmosphere';
...
function getWindowFactories() {
  ...
  return window.__sdsKonveyorAtmosphereMaterialFactories ?? null;
}

function exposeSummary(summary) {
  if (typeof window !== 'undefined') {
    window.__sdsKonveyorAtmosphereMaterialAdapter = summary;
  }
}
...
export function shouldApplyKonveyorAtmosphere(search = getWindowSearch()) {
  return shouldApplyKonveyorRendererFlag(search, FLAG_PARAM);
}

export function createKonveyorAtmosphereMaterial(kind, factoryName, {
  createDefaultMaterial,
  search = getWindowSearch(),
  factories = getWindowFactories(),
  context = {},
} = {}) {
  if (!shouldApplyKonveyorAtmosphere(search)) {
