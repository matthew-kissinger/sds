...
export function createWebGpuCloudLayerNodeMaterial(
  { MeshBasicNodeMaterial, DoubleSide, TSL },
  { name = 'webgpu-node-cloud-layer', uniforms = null } = {}
) {
  ...
  material.userData.webgpuCloudLayerNodeUniforms = {
    coverage,
    edgeFade,
    noiseScale,
    timeSeconds,
  ...
  material.userData.webgpuCloudSunColorSource = 'skyFog.sunColor';
  return material;
}

export function createWebGpuCloudLayerNodeMaterialResult(webGpuModules, options = {}) {
  const material = createWebGpuCloudLayerNodeMaterial(webGpuModules, options);
  const controls = createWebGpuCloudLayerNodeMaterialControls(material);
  return { material, controls };
}

export function createWebGpuCloudLayerMaterialFactories(webGpuModules) {
  return {
    createCloudLayerMaterial: (context = {}) =>
      createWebGpuCloudLayerNodeMaterialResult(webGpuModules, {
        uniforms: context.uniforms,
      }),
  };
}

function createWebGpuCloudLayerNodeMaterialControls(material) {
  const nodes = material.userData.webgpuCloudLayerNodeUniforms;
  return {
    nodes,
    update(state = {}) {
