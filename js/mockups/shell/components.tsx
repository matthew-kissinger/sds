/**
 * Cycle 51 P1: shared presentation building blocks for the bake-off. Skins
 * compose these so the world render, the dog avatar, and the loading bar look
 * consistent across prototypes while each skin owns its layout and chrome.
 */
import { useMemo, type CSSProperties } from 'react';
import { pastoral } from '../../components/ui/tokens';
import type { World, Dog } from './data';

/** Absolute-fill scene backdrop: the in-repo render over a gradient fallback. */
 ...
 * div, not a button) so skins can nest it inside their own clickable pill or
...
export function DogAvatar({
  ...
  return (
    ...
    >
      <img src={dog.portrait} alt={dog.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
    </div>
  );
}

/** The calm pastoral loading bar, driven by the calibrated stage progress. */
  ...
  return (
    <div style={{ width: '100%', maxWidth: width }}>
      <div style={{ height: 10, borderRadius: 999, background: pastoral.glassWarm, border: `1px solid ${pastoral.glassWarmBorder}`, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${pct}%`, background: accent ?? pastoral.accentMeadow, borderRadius: 999, transition: 'width 200ms cubic-bezier(0.25, 0.8, 0.35, 1)' }} />
      </div>
      ...
      {motes.map((m, i) => (
        <span
          ...
          style={{
            ...
            animation: reducedMotion ? 'none' : `mock-mote ${m.dur}s ease-in-out ${m.delay}s infinite`,
          }}
        />
      ))}
    </div>
